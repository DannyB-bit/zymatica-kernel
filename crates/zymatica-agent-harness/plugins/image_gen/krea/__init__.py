...
# Zymatica uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
        ...
        **kwargs: Any,
        ...
        if managed is not None:
            ...
            if not auth_token:
                return error_response(
                    error=(
                        "KREA_API_KEY not set. Run `zymatica tools` → Image "
                        "Generation → Krea to configure, get a key at "
                        "https://www.krea.ai/settings/api-tokens, or sign in to "
                        "a Zymatica account with the managed Krea gateway enabled "
                        "(`zymatica setup`)."
                    ),
        ...
        headers = {
            ...
            "User-Agent": "Zymatica-Agent/1.0 (krea-image-gen)",
        }
        if managed is not None:
        ...
        try:
            ...
            # the Zymatica Portal, or the gateway's shared Krea key hit its
            # concurrency cap (429).
            if managed is not None and 400 <= status < 500:
                hint = (
                    ...
                    if status == 429
                        ...
                        "Zymatica Portal's Krea gateway. Set KREA_API_KEY to use "
                        "Krea directly, or pick a different model via "
                        "`zymatica tools` → Image Generation."
                    )
                )
                return error_response(
                    error=(
