"""OpenRouter-compatible image generation backend (OpenRouter + Zymatica Portal).

Both OpenRouter and the Zymatica Portal inference endpoint speak the same
...
Zymatica Portal proxies OpenRouter, so one implementation services both — we only
swap the resolved ``(base_url, api_key)``. Credentials are resolved through the
agent's existing :func:`~zymatica_cli.runtime_provider.resolve_runtime_provider`,
...
# image_gen.model from ``zymatica tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Zymatica Portal). The two differ only
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``zymatica tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `zymatica tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against Zymatica Portal).
            "HTTP-Referer": "https://github.com/DannyB-bit/Zymatica.space-BODY",
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="nous",
            display_name="Zymatica Portal",
            runtime_name="nous",
            config_key="nous",
            model_env_var="NOUS_IMAGE_MODEL",
            setup_schema={
                "name": "Zymatica Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Zymatica Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Zymatica Portal image gen providers."""
