"""NousDashboardAuthProvider — Zymatica Portal OAuth (authorization-code + PKCE).
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      ZYMATICA_DASHBOARD_PORTAL_URL       — defaults to
                                          ``https://portal.zymatica.space``
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``zymatica-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``zymatica_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via ZYMATICA_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://portal.zymatica.space"
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    """Zymatica Portal OAuth via authorization-code + PKCE (S256)."""

    name = "nous"
    display_name = "Zymatica Team"
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"zymatica_session_pkce\"]``
        ...
        cookie_payload = {
            "zymatica_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. Zymatica Portal
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between ZYMATICA_DASHBOARD_PORTAL_URL / ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "Zymatica Portal token missing oauth_contract_version claim "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``ZYMATICA_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("ZYMATICA_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    configured (either via ``ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``zymatica_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "Zymatica Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Zymatica Agent "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"ZYMATICA_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The Zymatica Portal "
