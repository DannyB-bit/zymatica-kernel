...
A standards-compliant OpenID Connect Relying Party for the ``zymatica dashboard``
OAuth gate. Unlike the bundled ``nous`` provider (which encodes Zymatica Portal's
...
:class:`~zymatica_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``zymatica_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~zymatica_cli.dashboard_auth.Session`.
...
provider verifies its *access* token because Zymatica Portal mints a custom JWT
...
same precedence convention as the ``nous`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          issuer: https://auth.example.com/application/o/zymatica/   # required
          client_id: zymatica-dashboard                              # required
          scopes: "openid profile email"                           # optional
          # client_secret: set ONLY for a confidential client. It is a
          # credential — prefer the env var / ~/.zymatica/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    ZYMATICA_DASHBOARD_OIDC_ISSUER
    ZYMATICA_DASHBOARD_OIDC_CLIENT_ID
    ZYMATICA_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    ZYMATICA_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Zymatica does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``ZYMATICA_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "ZYMATICA_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "ZYMATICA_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("ZYMATICA_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.zymatica/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "ZYMATICA_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(ZYMATICA_DASHBOARD_OIDC_ISSUER + ZYMATICA_DASHBOARD_OIDC_CLIENT_ID) "
