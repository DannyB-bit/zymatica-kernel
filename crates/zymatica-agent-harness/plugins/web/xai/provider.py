...
``{title, url, description, position}`` rows every other Zymatica web
provider produces.
...
prefers Zymatica-managed xAI Grok OAuth (via ``zymatica auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.zymatica/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``zymatica tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `zymatica auth` to sign in with "
        ...
        headers = {
            ...
            "User-Agent": zymatica_xai_user_agent(),
        }
