...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``zymatica tools`` setup flow.
    """
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_nous_tools_enabled():
        message += (
            " With your Zymatica subscription you can also use the Tool Gateway. "
            "run `zymatica tools` and select Nous Subscription as the web provider."
        )
    else:
