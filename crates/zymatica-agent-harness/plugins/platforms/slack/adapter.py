...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `zymatica slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_zymatica_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Zymatica does not currently need to route them into the agent loop.
            ...
            # /model work directly without /zymatica prefix). A single regex
            ...
            # manifest (see `zymatica slack manifest`). In Socket Mode, Slack
            ...
            from zymatica_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/zymatica$")

            @self._app.command(_slash_pattern)
            async def handle_zymatica_command(ack, command):
            ...
            for _action_id in (
                "zymatica_approve_once",
                "zymatica_approve_session",
                "zymatica_approve_always",
                "zymatica_deny",
            ):
            ...
            for _action_id in (
                "zymatica_confirm_once",
                "zymatica_confirm_always",
                "zymatica_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("zymatica_feedback")(self._handle_feedback_action)
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Zymatica handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Zymatica session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Zymatica owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        if original_text.startswith("!"):
            try:
                from zymatica_cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@zymatica`` still resolve.
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "zymatica_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied:
                actions.append({
                    ...
                    "action_id": "zymatica_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "zymatica_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zymatica_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "zymatica_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "zymatica_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zymatica_confirm_once": "once",
            "zymatica_confirm_always": "always",
            "zymatica_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zymatica_approve_once": "once",
            "zymatica_approve_session": "session",
            "zymatica_approve_always": "always",
            "zymatica_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/zymatica <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/zymatica
        ...
        if slash_name in {"zymatica", ""}:
            # Legacy /zymatica <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from zymatica_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/zymatica   ``).
        ...
        # "Running /cmd…" ack shown by handle_zymatica_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/zymatica <question>" must produce public replies so
...
# ``zymatica_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``zymatica_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ZYMATICA_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from zymatica_cli.slack_cli import _build_full_manifest
            from zymatica_constants import get_zymatica_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Zymatica",
                bot_description="Your Zymatica agent on Slack",
            )
            target = Path(get_zymatica_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `zymatica slack manifest --write` anytime to refresh after "
                "Zymatica adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `zymatica update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://zymatica.space/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``zymatica_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'zymatica-agent[slack]'",
        # Interactive setup wizard — replaces zymatica_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in zymatica_cli/gateway.py.
