"""
``zymatica photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Zymatica
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `zymatica photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Zymatica Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``zymatica photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Zymatica Agent" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.zymatica/.env,
    ...
    print("  Start the gateway:  zymatica gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `zymatica photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`zymatica photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.zymatica/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `zymatica photon telemetry on` / `zymatica photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.zymatica/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  zymatica gateway restart")
    return 0
...
#
# `zymatica gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``zymatica photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `zymatica gateway setup` wizard."""
