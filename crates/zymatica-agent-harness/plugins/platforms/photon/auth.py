...
Credential storage mirrors every other Zymatica channel:

    * runtime SDK creds  -> ``~/.zymatica/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.zymatica/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a zymatica dependency
...
# registers Zymatica as its own client_id.
...
# Default name of the project Zymatica provisions for the operator.
DEFAULT_PROJECT_NAME = "Zymatica Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of zymatica-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.zymatica/auth.json`` honouring the active Zymatica profile."""
    try:
        from zymatica_constants import get_zymatica_home
        return Path(get_zymatica_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.zymatica")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.zymatica/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.zymatica/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.zymatica/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: zymatica_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `zymatica photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `zymatica photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `zymatica photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `zymatica photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `zymatica photon setup`)"
