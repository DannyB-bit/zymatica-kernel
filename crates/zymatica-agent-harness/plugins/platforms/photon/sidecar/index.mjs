// Zymatica Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Zymatica): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Zymatica -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Zymatica-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `zymatica photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Zymatica can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Zymatica'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Zymatica allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Zymatica, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-zymatica-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Zymatica supplied them so a known-good
