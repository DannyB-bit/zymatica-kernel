...
Small Node helper that bridges Zymatica Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Zymatica is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Zymatica-Sidecar-Token`)
...
The Zymatica plugin's `zymatica photon setup` command runs `npm install`
here automatically.
