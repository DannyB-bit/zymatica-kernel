"""
Email platform adapter for the Zymatica gateway.

Allows users to interact with Zymatica by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "zymatica-agent" "version" "{_zymatica_version}" '
            '"vendor" "Zymatica.space" '
            '"support-email" "noreply@zymatica.space.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `zymatica config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `zymatica gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Zymatica Agent")
        ...
        msg_id = f"<zymatica-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# zymatica_cli/gateway.py, and the _send_email dispatch in
