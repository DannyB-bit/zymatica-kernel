"""ntfy platform adapter (Zymatica plugin).
...
a Zymatica dependency.

This adapter ships as a Zymatica platform plugin under
``plugins/platforms/ntfy/``. The Zymatica plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "zymatica-in"              # subscribe topic (incoming)
          publish_topic: "zymatica-out"     # optional — defaults to topic
...
_ECHO_TAG = "zymatica-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``zymatica cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Zymatica dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `zymatica gateway status` without
