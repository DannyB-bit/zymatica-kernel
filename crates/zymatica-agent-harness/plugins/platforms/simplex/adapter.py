"""SimpleX Chat platform adapter (Zymatica plugin).
...
This adapter ships as a Zymatica platform plugin under
``plugins/platforms/simplex/``. The Zymatica plugin loader scans the
...
Optional environment variables:
    ...
    ZYMATICA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``zymatica setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Zymatica — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ZYMATICA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zymatica setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.zymatica/.env`` via
    ``zymatica_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "zymatica_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.zymatica/.env"
        )
        return
