...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {"title": "Zymatica", "text": normalized},
        }
    ...
    async def _create_and_stream_card(
        ...
        *,
        ...
        try:
            ...
            out_track_id = f"zymatica_{uuid.uuid4().hex[:12]}"
...
# dict in zymatica_cli/gateway.py, and the _send_dingtalk dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces zymatica_cli/setup.py-era _setup_dingtalk + the static
    _PLATFORMS["dingtalk"] dict in zymatica_cli/gateway.py. CLI helpers are
    ...
    if method == 0:
        try:
            from zymatica_cli.dingtalk_auth import dingtalk_qr_auth
