"""
Microsoft Teams platform adapter for Zymatica Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Zymatica"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; ZymaticaAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        zymatica_action = data.get("zymatica_action", "")
        session_key = data.get("session_key", "")

        if not zymatica_action or not session_key:
        ...
        choice = choice_map.get(zymatica_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="zymatica_approve",
            data={**btn_data_base, "zymatica_action": "approve_once"}, style="positive",
        )]
        if not smart_denied:
            actions.append(ExecuteAction(
                title="Allow Session", verb="zymatica_approve",
                data={**btn_data_base, "zymatica_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="zymatica_approve",
                    data={**btn_data_base, "zymatica_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="zymatica_approve",
            data={**btn_data_base, "zymatica_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Zymatica\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.zymatica/.env")
