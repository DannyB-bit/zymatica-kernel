...
# ``model_tools`` import time, so ``zymatica status``, ``zymatica chat``, etc.
...
# ``zymatica_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Zymatica is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a zymatica MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "zymatica-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ZYMATICA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _zymatica_home = _get_zymatica_home()
        except (ModuleNotFoundError, ImportError):
            _zymatica_home = _Path.home() / ".zymatica"
        self._thread_count_store = _ThreadCountStore(
            _zymatica_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Zymatica is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ZYMATICA_HOME", str(_Path.home() / ".zymatica"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ZYMATICA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a zymatica MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Zymatica is thinking…' marker message.
        ...
        "Zymatica is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Zymatica is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Zymatica is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Zymatica is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``zymatica setup``.

    The setup wizard at ``zymatica_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.zymatica/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. zymatica-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.zymatica/.env")
    print_info("Restart the gateway: zymatica gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``zymatica cron`` running as a
    separate process from ``zymatica gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Zymatica plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'zymatica-agent[google_chat]'",
        ...
        # the "Zymatica is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Zymatica is thinking…' marker message "
