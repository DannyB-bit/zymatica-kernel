"""
LINE Messaging API platform adapter for Zymatica Agent.
...
to Zymatica Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ZYMATICA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            zymatica_home = Path(get_zymatica_home()).resolve()
        except Exception:
            zymatica_home = Path.home().joinpath(".zymatica").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``zymatica status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``zymatica status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zymatica setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.zymatica/.env`` via ``zymatica_cli.config``.
    """
    print()
    ...
    try:
        from zymatica_cli.config import get_env_var, set_env_var
    except ImportError:
        print("zymatica_cli.config not available; set LINE_* vars manually in ~/.zymatica/.env")
        return
