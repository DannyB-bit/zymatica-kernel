...
Auto-tracks and cleans up ephemeral files created during Zymatica Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ZYMATICA_HOME` and `/tmp/zymatica-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZYMATICA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ZYMATICA_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `ZYMATICA_HOME` or `/tmp/zymatica-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ZYMATICA_HOME/disk-cleanup/` is itself excluded
- `$ZYMATICA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
