"""disk_cleanup — ephemeral file cleanup for Zymatica Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ZYMATICA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ZYMATICA_HOME and /tmp/zymatica-*
Never touches: ~/.zymatica/logs/ or any system directory.
"""
...
try:
    ...
    def get_zymatica_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ZYMATICA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".zymatica").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ZYMATICA_HOME/logs/``."""
    return get_zymatica_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ZYMATICA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ZYMATICA_HOME or ``/tmp/zymatica-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    zymatica_home = get_zymatica_home()
    try:
        path.resolve().relative_to(zymatica_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/zymatica-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("zymatica-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "zymatica-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $ZYMATICA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ZYMATICA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        zymatica_home = get_zymatica_home()
        for parent in ("cron", "cronjobs"):
            base = zymatica_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ZYMATICA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ZYMATICA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Zymatica checkout, venv,
    # and desktop build under ZYMATICA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    zymatica_home = get_zymatica_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in zymatica_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    zymatica_home = get_zymatica_home()
    try:
        rel = path.resolve().relative_to(zymatica_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "zymatica-agent",
        }:
            return None
        ...
        # Path isn't under ZYMATICA_HOME (e.g. /tmp/zymatica-*) — fall through.
        pass
