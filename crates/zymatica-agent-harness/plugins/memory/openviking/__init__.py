...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Zymatica peer ID in OpenViking (default: zymatica)
...
_DEFAULT_AGENT = "zymatica"
_AGENT_PROMPT_LABEL = "Zymatica peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "ZYMATICA_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Zymatica slash-skill scaffolding before sending content to OpenViking.
...
def _load_zymatica_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("ZYMATICA_HOME", "")).expanduser() if os.environ.get("ZYMATICA_HOME") else Path.home() / ".zymatica"
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this Zymatica run."
    )
...
def _save_zymatica_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Zymatica session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Zymatica only", "write values only to Zymatica .env"),
    ...
    _save_zymatica_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Zymatica .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See zymatica-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_zymatica_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Zymatica peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, zymatica_home: str, config: dict) -> None:
        ...
        from zymatica_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        zymatica_home_path = Path(zymatica_home)
        env_path = zymatica_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_zymatica_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Zymatica run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Zymatica' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Zymatica canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See zymatica-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/zymatica/.overview.md).
