...
```bash
zymatica memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into Zymatica, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:
```bash
zymatica config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.zymatica/.env
```
...
| `OPENVIKING_AGENT` | `zymatica` | Zymatica peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Zymatica lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Zymatica sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Zymatica built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Zymatica action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Zymatica
...
file URIs, such as
`viking://user/peers/zymatica/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/zymatica/memories/preferences/mem_abc123.md`. Files
