...
```bash
zymatica memory setup    # select "holographic"
```

Or manually:
```bash
zymatica config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.zymatica-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ZYMATICA_HOME/memory_store.db` | SQLite database path |
