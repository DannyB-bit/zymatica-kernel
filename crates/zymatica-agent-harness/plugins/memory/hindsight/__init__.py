...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: zymatica)
...
Or via $ZYMATICA_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Zymatica can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ZYMATICA_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_zymatica_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "zymatica": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "zymatica"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Zymatica config."""
    profile = config.get("profile", "zymatica")
    return str(profile or "zymatica")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Zymatica profile name (from agent_identity)
      {workspace} — Zymatica workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``zymatica-{user}`` with no user becomes ``zymatica``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Zymatica Agent and the User"
    ...
    def save_config(self, values, zymatica_home):
        """Write config to $ZYMATICA_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(zymatica_home) / "hindsight"
    ...
    def post_setup(self, zymatica_home: str, config: dict) -> None:
        ...
        from zymatica_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(zymatica_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "zymatica")
        ...
        self.save_config(provider_config, zymatica_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(zymatica_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(zymatica_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "zymatica"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: zymatica-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Zymatica Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "zymatica"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "zymatica"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "zymatica", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "zymatica")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Zymatica Agent and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Zymatica as a non-root user, or switch "
                    "to cloud / local_external mode via 'zymatica memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Zymatica get sluggish. (issue #13125)
                try:
            ...
            def _start_daemon():
                import traceback
                log_dir = get_zymatica_home() / "logs"
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See zymatica-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Zymatica created/used that client on the shared async loop,
