...
```bash
zymatica memory setup    # select "byterover"
```

Or manually:
```bash
zymatica config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.zymatica/.env
```
...
Working directory: `$ZYMATICA_HOME/byterover/` (profile-scoped).
