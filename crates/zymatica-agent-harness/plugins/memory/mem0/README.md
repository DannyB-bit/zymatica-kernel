...
```bash
zymatica memory setup    # select "mem0"
```

Or manually:
```bash
zymatica config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.zymatica/.env
```

## Config

Behavioral settings live in `$ZYMATICA_HOME/mem0.json` (set them via `zymatica memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.zymatica/.env`.
...
| `user_id` | `zymatica-user` | User identifier on Mem0 |
| `agent_id` | `zymatica` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   zymatica memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   zymatica memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.zymatica/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.zymatica/.env
   ```
   or in `$ZYMATICA_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Zymatica session and call `mem0_search` — it connects to your server.
...
```bash
zymatica memory setup mem0 --mode oss \
...
```bash
zymatica memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ZYMATICA_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.zymatica/mem0_qdrant"}}
  }
}
```
...
```bash
zymatica memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
zymatica memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$ZYMATICA_HOME/mem0.json`).
