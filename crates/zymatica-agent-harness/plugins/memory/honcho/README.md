...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/zymatica>
...
```bash
zymatica memory setup honcho   # configure Honcho directly (works on a fresh install)
zymatica memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
zymatica config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.zymatica/.env
```

> `zymatica honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `zymatica memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `zymatica model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ZYMATICA_HOME/honcho.json` | Profile-local (isolated Zymatica instances) |
| 2 | `~/.zymatica/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Zymatica profile: `zymatica` (default) or `zymatica_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `zymatica honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Zymatica session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`zymatica-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`zymatica`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-zymatica-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening zymatica in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Zymatica session ID (timestamp + hex). Every `zymatica` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Zymatica profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`zymatica`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Zymatica profile: `zymatica` (default) or `zymatica_<profile>` (e.g. `zymatica -p coder` -> host key `zymatica_coder`). Older `zymatica.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ZYMATICA_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `zymatica-agent`) |
...
|---------|-------------|
| `zymatica memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `zymatica honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `zymatica memory setup`) |
| `zymatica honcho status` | Show resolved config for active profile |
| `zymatica honcho enable` / `disable` | Toggle Honcho for active profile |
| `zymatica honcho mode <mode>` | Change recall or observation mode |
| `zymatica honcho peer --user <name>` | Update user peer name |
| `zymatica honcho peer --ai <name>` | Update AI peer name |
| `zymatica honcho tokens --context <N>` | Set context token budget |
| `zymatica honcho tokens --dialectic <N>` | Set dialectic max chars |
| `zymatica honcho map <name>` | Map current directory to a session name |
| `zymatica honcho sync` | Create host blocks for all Zymatica profiles |
