...
``zymatica://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Zymatica OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "zymatica-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``zymatica-desktop`` / ``zymatica-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Zymatica.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ZYMATICA_HOME override can't reach.
