...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with zymatica-agent)
2. User-installed providers: ``$ZYMATICA_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_zymatica_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_zymatica_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_zymatica_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ZYMATICA_HOME/plugins/`` or None if unavailable."""
    try:
        from zymatica_constants import get_zymatica_home
        d = get_zymatica_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$ZYMATICA_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _zymatica_user_memory.<name>.cli, usually
