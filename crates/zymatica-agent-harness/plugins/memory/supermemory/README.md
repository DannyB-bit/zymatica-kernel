...
- Supermemory API key from [app.supermemory.ai/integrations?connect=zymatica](http://app.supermemory.ai/integrations?connect=zymatica)

## Setup

```bash
zymatica memory setup    # select "supermemory"
```

Or manually:

```bash
zymatica config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.zymatica/.env
```

## Config

Config file: `$ZYMATICA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zymatica` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zymatica-{identity}` → `zymatica-coder`). |
...
All Supermemory API calls send `x-sm-source: zymatica`, and document writes stamp
`metadata.sm_source: zymatica`. This is a **functional routing key, not telemetry**:
it groups Zymatica-written memories into a dedicated "Zymatica" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Zymatica profile:

```json
{
  "container_tag": "zymatica-{identity}"
}
```

For a profile named `coder`, this resolves to `zymatica-coder`. The default profile resolves to `zymatica-default`. Without `{identity}`, all profiles share the same container.
