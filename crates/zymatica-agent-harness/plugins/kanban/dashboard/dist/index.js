/**
 * Zymatica Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.zymatica/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__ZYMATICA_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__ZYMATICA_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "zymatica-kanban-dot-triage",
    todo: "zymatica-kanban-dot-todo",
    ready: "zymatica-kanban-dot-ready",
    running: "zymatica-kanban-dot-running",
    blocked: "zymatica-kanban-dot-blocked",
    done: "zymatica-kanban-dot-done",
    archived: "zymatica-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-zymatica-task";
  ...
  const DOCS_URL = "https://zymatica-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://zymatica-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "zymatica.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="zymatica-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])zymatica-kanban-md-code\1/i.test(attrs)) {
      return ' class="zymatica-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "zymatica-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "zymatica-kanban-md",
  ...
  // standard `drop` event and our `zymatica-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("zymatica-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("zymatica-kanban-column--drop");
          if (target) target.classList.add("zymatica-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("zymatica-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("zymatica-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("zymatica-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__ZYMATICA_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "zymatica-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "zymatica-kanban-attention",
        "zymatica-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "zymatica-kanban-attention-bar" },
        h("span", { className: "zymatica-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "zymatica-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "zymatica-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "zymatica-kanban-attention-row",
                  "zymatica-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "zymatica-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "zymatica-kanban-attention-row-id" }, task.id),
                h("span", { className: "zymatica-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "zymatica-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "zymatica-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "zymatica-kanban-diag-action-btn",
      action.suggested ? "zymatica-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " zymatica-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".zymatica-kanban-drawer-comment-row input, .zymatica-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "zymatica-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("zymatica-kanban-diag", sevClass) },
      h("div", { className: "zymatica-kanban-diag-header" },
        h("span", { className: "zymatica-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "zymatica-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "zymatica-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "zymatica-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "zymatica-kanban-diag-data-row" },
                  h("span", { className: "zymatica-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "zymatica-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "zymatica-kanban-diag-data-row" },
                h("span", { className: "zymatica-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "zymatica-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "zymatica-kanban-diag-reassign-row" },
            h("span", { className: "zymatica-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "zymatica-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "zymatica-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "zymatica-kanban-diag-msg",
              msg.ok ? "zymatica-kanban-diag-msg--ok" : "zymatica-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "zymatica-kanban-section" },
      h("div", { className: "zymatica-kanban-section-head-row" },
        h("span", { className: "zymatica-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "zymatica-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "zymatica-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "zymatica-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "zymatica-kanban-docs-link",
      title: "Open Zymatica Kanban docs in a new tab",
      "aria-label": "Zymatica Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `zymatica kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "zymatica-kanban-msg-ok" : "zymatica-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "zymatica-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "zymatica-kanban-boardswitcher" },
      h("div", { className: "zymatica-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "zymatica-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "zymatica-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "zymatica-kanban-dialog-title" },
        ...
        h("div", { className: "zymatica-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Zymatica profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "zymatica-kanban-bulk" },
      h("span", { className: "zymatica-kanban-bulk-count" },
      ...
      h("div", { className: "zymatica-kanban-bulk-priority",
      ...
      h("div", { className: "zymatica-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Zymatica profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "zymatica-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zymatica-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("zymatica-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "zymatica-kanban-trash",
        dragOver ? "zymatica-kanban-trash--drop" : "",
        props.draggingTaskId ? "zymatica-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "zymatica-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "zymatica-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".zymatica-kanban-card")) return true;
      if (target.closest && target.closest(".zymatica-kanban-column-add")) return true;
      if (target.closest && target.closest(".zymatica-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("zymatica-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("zymatica-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".zymatica-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "zymatica-kanban-columns",
        isScrollable ? "zymatica-kanban-columns--scrollable" : "",
        isPanning ? "zymatica-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("zymatica-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("zymatica-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "zymatica-kanban-column",
        dragOver ? "zymatica-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "zymatica-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "zymatica-kanban-col-check",
        ...
        h("span", { className: cn("zymatica-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "zymatica-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "zymatica-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "zymatica-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "zymatica-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "zymatica-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "zymatica-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "zymatica-kanban-lane" },
                  h("div", { className: "zymatica-kanban-lane-head" },
                    h("span", { className: "zymatica-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "zymatica-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "zymatica-kanban-card--stale-red";
    if (age >= tier.amber) return "zymatica-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".zymatica-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "zymatica-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "zymatica-kanban-card",
        props.selected ? "zymatica-kanban-card--selected" : "",
        props.failed ? "zymatica-kanban-card--failed" : "",
        props.draggingSource ? "zymatica-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "zymatica-kanban-card-content" },
          h("div", { className: "zymatica-kanban-card-row" },
            h("label", {
              className: "zymatica-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "zymatica-kanban-card-check",
            ...
            h("span", { className: "zymatica-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or zymatica kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "zymatica-kanban-warning-badge",
                    "zymatica-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "zymatica-kanban-needs-assignee",
          ...
          h("div", { className: "zymatica-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "zymatica-kanban-card-row zymatica-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "zymatica-kanban-assignee",
                            title: `Assigned to Zymatica profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "zymatica-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "zymatica-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "zymatica-kanban-ago",
  ...
  // centered modal reusing the zymatica-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "zymatica-kanban-dialog zymatica-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Zymatica profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Zymatica profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "zymatica-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "zymatica-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "zymatica-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "zymatica-kanban-drawer-close",
        ...
        data ? h("div", { className: "zymatica-kanban-drawer-comment-foot" },
          h("div", {
            className: "zymatica-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "zymatica-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "zymatica-kanban-section" },
      h("div", { className: "zymatica-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "zymatica-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "zymatica-kanban-drawer-body" },
      h("div", { className: "zymatica-kanban-drawer-title" },
        h("span", { className: cn("zymatica-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "zymatica-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "zymatica-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "zymatica-kanban-section" },
            h("div", { className: "zymatica-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "zymatica-kanban-section" },
            h("div", { className: "zymatica-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "zymatica-kanban-done-no-result zymatica-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "zymatica-kanban-section" },
            h("div", { className: "zymatica-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "zymatica-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "zymatica-kanban-section" },
        h("div", { className: "zymatica-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "zymatica-kanban-comment" },
            h("div", { className: "zymatica-kanban-comment-head" },
              h("span", { className: "zymatica-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "zymatica-kanban-diag-action-btn",
      ...
      h("div", { className: "zymatica-kanban-section" },
        h("div", { className: "zymatica-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "zymatica-kanban-comment" },
                h("div", { className: "zymatica-kanban-comment-head" },
                  h("span", { className: "zymatica-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "zymatica-kanban-comment-ago" },
      ...
      h("div", { className: "zymatica-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "zymatica-kanban-event",
              isDiag ? "zymatica-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "zymatica-kanban-event-header" },
                  h("span", { className: "zymatica-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "zymatica-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "zymatica-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "zymatica-kanban-event-header-plain" },
                  h("span", { className: "zymatica-kanban-event-kind" }, e.kind),
                  h("span", { className: "zymatica-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "zymatica-kanban-event-phantom-row" },
                  h("span", { className: "zymatica-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "zymatica-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "zymatica-kanban-event-payload" },
    ...
    return h("div", { className: "zymatica-kanban-section" },
      h("div", { className: "zymatica-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "zymatica-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `zymatica-kanban-run--${r.outcome || r.status || "ended"}`
          : "zymatica-kanban-run--active";
        return h("div", { key: r.id, className: cn("zymatica-kanban-run", outcomeClass) },
          h("div", { className: "zymatica-kanban-run-head" },
            h("span", { className: "zymatica-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "zymatica-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "zymatica-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "zymatica-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "zymatica-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "zymatica-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "zymatica-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "zymatica-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "zymatica-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "zymatica-kanban-pre zymatica-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "zymatica-kanban-meta-row" },
      h("span", { className: "zymatica-kanban-meta-label" }, props.label),
      h("span", { className: "zymatica-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "zymatica-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zymatica-kanban-meta-row" },
        h("span", { className: "zymatica-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "zymatica-kanban-meta-value zymatica-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "zymatica-kanban-meta-row" },
        h("span", { className: "zymatica-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "zymatica-kanban-meta-value zymatica-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "zymatica-kanban-section" },
      h("div", { className: "zymatica-kanban-section-head-row" },
        h("span", { className: "zymatica-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "zymatica-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "zymatica-kanban-section" },
      h("div", { className: "zymatica-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "zymatica-kanban-deps-row" },
        h("span", { className: "zymatica-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "zymatica-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "zymatica-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "zymatica-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "zymatica-kanban-dep-chip-x",
      ...
      h("div", { className: "zymatica-kanban-deps-row" },
        h("span", { className: "zymatica-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "zymatica-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "zymatica-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "zymatica-kanban-msg-ok"
          : "zymatica-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "zymatica-kanban-section" },
      ...
      h("div", { className: "zymatica-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "zymatica-kanban-home-sub zymatica-kanban-home-sub--on"
              : "zymatica-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__ZYMATICA_PLUGINS__ && typeof window.__ZYMATICA_PLUGINS__.register === "function") {
    window.__ZYMATICA_PLUGINS__.register("kanban", KanbanPage);
  }
})();
