...
Optional Zymatica observability plugin that maps Zymatica observer hooks to
...
not replace Zymatica Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Zymatica emit NeMo Relay lifecycle events for
the work Zymatica already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Zymatica Agent can:

- Preserve Zymatica execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
zymatica plugins enable observability/nemo_relay
```

The `ZYMATICA_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ZYMATICA_HOME` that the
agent run will use:

```bash
env ZYMATICA_HOME=/tmp/zymatica-nemo-relay-test \
  zymatica plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ZYMATICA_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ZYMATICA_HOME` is the Zymatica profile/config home used by both
`zymatica plugins enable ...` and the later `zymatica chat ...` run. If unset,
Zymatica uses the user's default home, usually `~/.zymatica`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ZYMATICA_HOME=/tmp/zymatica-nemo-relay-test
zymatica plugins enable observability/nemo_relay
zymatica chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `zymatica` command you run is built from the
...
uv run zymatica plugins enable observability/nemo_relay
uv run zymatica chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/zymatica_agent-*.whl
...
The plugin can configure exporters directly from `ZYMATICA_NEMO_RELAY_*`
...
```bash
export ZYMATICA_NEMO_RELAY_ATOF_ENABLED=1
export ZYMATICA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ZYMATICA_NEMO_RELAY_ATIF_ENABLED=1
export ZYMATICA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ZYMATICA_NEMO_RELAY_ATOF_FILENAME`
- `ZYMATICA_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ZYMATICA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ZYMATICA_NEMO_RELAY_ATIF_AGENT_NAME`
- `ZYMATICA_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ZYMATICA_NEMO_RELAY_ATIF_MODEL_NAME`
- `ZYMATICA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Zymatica at it:

```bash
export ZYMATICA_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Zymatica Agent"
agent_version = "local"
```

When `ZYMATICA_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ZYMATICA_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`ZYMATICA_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Zymatica does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Zymatica keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Zymatica routes LLM and tool
...
Zymatica feature-detects the dynamic-plugin activation API available in NeMo Relay
0.6 and later. Configure native or worker plugins with Zymatica-owned
...
with this Zymatica-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Zymatica rejects
...
Zymatica activates these plugins before registering its managed LLM and tool
...
activation API, Zymatica logs an actionable warning and continues with the
...
For the full generic Zymatica middleware contract, see
...
export ZYMATICA_HOME=/tmp/zymatica-nemo-relay-docs/zymatica-home
mkdir -p "$ZYMATICA_HOME"

cat > "$ZYMATICA_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Zymatica session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ZYMATICA_NEMO_RELAY_ATOF_ENABLED=1
export ZYMATICA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/zymatica-nemo-relay-docs/subagent/atof
export ZYMATICA_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ZYMATICA_NEMO_RELAY_ATOF_MODE=overwrite
export ZYMATICA_NEMO_RELAY_ATIF_ENABLED=1
export ZYMATICA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/zymatica-nemo-relay-docs/subagent/atif
export ZYMATICA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ZYMATICA_NEMO_RELAY_ATIF_AGENT_NAME='Zymatica Agent E2E'
export ZYMATICA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ZYMATICA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"zymatica.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Zymatica Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Zymatica dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/zymatica-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/zymatica-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/zymatica-nemo-relay-docs/workdir/beta.txt
cd /tmp/zymatica-nemo-relay-docs/workdir

export ZYMATICA_NEMO_RELAY_ATOF_ENABLED=1
export ZYMATICA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/zymatica-nemo-relay-docs/parallel/atof
export ZYMATICA_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ZYMATICA_NEMO_RELAY_ATOF_MODE=overwrite
export ZYMATICA_NEMO_RELAY_ATIF_ENABLED=1
export ZYMATICA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/zymatica-nemo-relay-docs/parallel/atif
export ZYMATICA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ZYMATICA_NEMO_RELAY_ATIF_AGENT_NAME='Zymatica Agent E2E'
export ZYMATICA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Zymatica sessions map to `agent` scopes.
- Zymatica API request hooks map to `llm` scope start/end events.
- Zymatica tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Zymatica execution middleware to hand
...
Enable it for Zymatica:

```bash
export ZYMATICA_NEMO_RELAY_PLUGINS_TOML=/tmp/zymatica-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Zymatica routes execution
through these boundaries:

```text
Zymatica provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Zymatica provider adapter next_call(...)

Zymatica tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Zymatica tool dispatcher next_call(...)
```
...
middleware for a local Zymatica run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by the supported
0.x release range beginning with 0.5.

```bash
export ZYMATICA_HOME=/tmp/zymatica-middleware-test/zymatica-home
mkdir -p "$ZYMATICA_HOME" /tmp/zymatica-middleware-test/nemo-relay
...
cat > /tmp/zymatica-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/zymatica-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/zymatica-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Zymatica Middleware E2E"
agent_version = "local"
...
export ZYMATICA_NEMO_RELAY_PLUGINS_TOML=/tmp/zymatica-middleware-test/nemo-relay/plugins.toml
