...
session-scoped state file under ``$ZYMATICA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ZYMATICA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_zymatica_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this zymatica install, leave it first —
    ...
    env = os.environ.copy()
    env["ZYMATICA_MEET_URL"] = url
    env["ZYMATICA_MEET_OUT_DIR"] = str(out)
    env["ZYMATICA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ZYMATICA_MEET_HEADED"] = "1"
    if auth_state:
        env["ZYMATICA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ZYMATICA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ZYMATICA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ZYMATICA_MEET_MODE"] = mode
    if realtime_model:
        env["ZYMATICA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ZYMATICA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ZYMATICA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ZYMATICA_MEET_REALTIME_KEY"] = realtime_api_key
