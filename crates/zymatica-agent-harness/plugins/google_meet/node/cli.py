"""`zymatica meet node ...` subcommand tree.

Wired into the existing ``zymatica meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``zymatica meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="zymatica-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``zymatica meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             zymatica meet node approve <name> ws://<host>:{args.port} {token}")
        try:
