...
Let the zymatica agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where zymatica runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `zymatica meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `zymatica meet` CLI |
...
| `cli.py` | `zymatica meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ZYMATICA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `zymatica meet node {run,list,approve,remove,status,ping}` |
...
zymatica meet install                                      # pip + Chromium
zymatica meet setup                                        # preflight
zymatica meet auth                                         # optional
zymatica meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
zymatica meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.zymatica/.env
zymatica meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
zymatica meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
zymatica meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.zymatica/.env
zymatica meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, zymatica will **not** switch your system audio input automatically — the
...
zymatica plugins enable google_meet
zymatica meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
zymatica meet node ping my-mac
