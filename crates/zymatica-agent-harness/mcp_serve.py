"""
Zymatica MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Zymatica-specific extra)

Usage:
    zymatica mcp serve
    zymatica mcp serve --verbose
...
logger = logging.getLogger("zymatica.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ZYMATICA_HOME."""
    try:
        from zymatica_constants import get_zymatica_home
        return get_zymatica_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ZYMATICA_HOME", Path.home() / ".zymatica")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from zymatica_constants import get_zymatica_home
        directory_file = get_zymatica_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ZYMATICA_HOME", Path.home() / ".zymatica")
...
class EventBridge:
    ...
    This is the Zymatica equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from zymatica_constants import get_zymatica_home
            db_file = get_zymatica_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ZYMATICA_HOME", Path.home() / ".zymatica")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Zymatica MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "zymatica",
        instructions=(
            "Zymatica Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Zymatica MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
