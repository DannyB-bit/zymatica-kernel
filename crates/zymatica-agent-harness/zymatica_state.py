#!/usr/bin/env python3
"""
SQLite State Store for Zymatica Agent.
...
try:
    import zymatica_core
except ImportError:
    zymatica_core = None

if zymatica_core is not None:
    class StateDBRow:
        def __init__(self, dictionary, column_names):
            self._dict = dictionary
            self._keys = column_names
    ...
    def _raise_sqlite_error(e):
        err_msg = str(e)
        if "UNIQUE constraint failed" in err_msg or "FOREIGN KEY constraint failed" in err_msg:
    ...
    class StateDBCursor:
        ...
        def execute(self, sql, params=None):
            is_query = sql.strip().upper().startswith(("SELECT", "PRAGMA", "WITH", "EXPLAIN"))
            if is_query:
                try:
                    self.results = self.db.query(sql, params)
                except Exception as e:
                    _raise_sqlite_error(e)
            else:
                try:
                    self.rowcount = self.db.execute(sql, params)
                    self.results = []
                except Exception as e:
                    _raise_sqlite_error(e)
            self.index = 0
            self.lastrowid = self.db.last_insert_rowid()
            return self
        ...
        def fetchone(self):
            if self.index < len(self.results):
                ...
                return StateDBRow(res, column_names)
            return None
    ...
    class StateDBConnection:
        def __init__(self, db_path):
            self.db = zymatica_core.StateDB(db_path)
            self.row_factory = None

        def execute(self, sql, params=None):
            cursor = StateDBCursor(self.db)
        ...
        def cursor(self):
            return StateDBCursor(self.db)

    _original_dumps = json.dumps
    _original_loads = json.loads

    def _native_dumps(obj, **kwargs):
        if not kwargs:
            try:
                return zymatica_core.json_encode(obj)
            except Exception:
                pass
        return _original_dumps(obj, **kwargs)

    def _native_loads(s, **kwargs):
        if not kwargs:
            try:
                return zymatica_core.json_decode(s)
            except Exception:
                pass
        return _original_loads(s, **kwargs)

    json.dumps = _native_dumps
    json.loads = _native_loads
...
DEFAULT_DB_PATH = get_zymatica_home() / "state.db"
...
# zymatica_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``zymatica_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see zymatica_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple zymatica processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            def _connect_and_init():
                is_mocked = sqlite3.connect.__module__ != "sqlite3"
                if zymatica_core is not None and not is_mocked:
                    self._conn = StateDBConnection(str(self.db_path))
                    self._init_schema()
                else:
                    self._conn = sqlite3.connect(
                        str(self.db_path),
            ...
            # ``zymatica_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `zymatica update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._zymatica_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._zymatica_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Zymatica surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``zymatica sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Zymatica, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Zymatica session.

        A Zymatica session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Zymatica session is already bound to any Telegram DM topic.
