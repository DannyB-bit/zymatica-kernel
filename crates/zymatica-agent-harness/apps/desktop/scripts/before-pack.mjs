 ...
 * product name (`Zymatica`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Zymatica'
 *
 * This is a hard failure with no obvious cause for the user — `zymatica desktop`
 ...
 * (the mac-unpacked Zymatica.app bundle) and Windows (win-unpacked), so we
