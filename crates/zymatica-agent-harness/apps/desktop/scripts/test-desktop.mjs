...
// no longer bundles the Zymatica Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Zymatica.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Zymatica'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Zymatica.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Zymatica'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ZYMATICA_HOME for non-sandboxed runs -- matches main.ts's
// resolveZymaticaHome(). On Windows it's %LOCALAPPDATA%\zymatica; elsewhere
// it's ~/.zymatica. The fresh-install sandbox launchFresh() sets its own
// ZYMATICA_HOME and never touches this.
const DEFAULT_ZYMATICA_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'zymatica')
  }
  return path.join(os.homedir(), '.zymatica')
})()
const VENV_ROOT = path.join(DEFAULT_ZYMATICA_HOME, 'zymatica-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'zymatica-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ZYMATICA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Zymatica-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Zymatica-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Zymatica-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Zymatica-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ZYMATICA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ZYMATICA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const zymaticaHome = path.join(sandbox, 'zymatica-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(zymaticaHome, { recursive: true })
  ...
  env.ZYMATICA_DESKTOP_CWD = cwd
  env.ZYMATICA_DESKTOP_IGNORE_EXISTING = '1'
  env.ZYMATICA_DESKTOP_TEST_MODE = 'fresh-install'
  env.ZYMATICA_DESKTOP_USER_DATA_DIR = userDataDir
  env.ZYMATICA_HOME = zymaticaHome
  delete env.ZYMATICA_DESKTOP_ZYMATICA
  delete env.ZYMATICA_DESKTOP_ZYMATICA_ROOT
  ...
  console.log(`  ZYMATICA_HOME: ${zymaticaHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(zymaticaHome, 'zymatica-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Zymatica Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of zymatica_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'zymatica-agent', 'zymatica_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Zymatica
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ZYMATICA_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ZYMATICA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
