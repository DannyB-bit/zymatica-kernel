#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Zymatica icon + version metadata onto the
// built Zymatica.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Zymatica.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `zymatica desktop`, the installer's
...
// path (which rebuilds via `zymatica desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Zymatica.exe>
//
...
// Stamp the Zymatica icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Zymatica',
      FileDescription: 'Zymatica',
      CompanyName: 'Zymatica Team',
      LegalCopyright: 'Copyright (c) 2026 Zymatica Team'
    }
  })

  console.log('[set-exe-identity] done — Zymatica icon + identity stamped')
}
