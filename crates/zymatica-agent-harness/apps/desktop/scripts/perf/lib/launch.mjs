...
//   · its own ZYMATICA_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · ZYMATICA_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated ZYMATICA_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate ZYMATICA_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = zymaticaHome ?? mkTemp('zymatica-perf-home-')
  const userData = userDataDir ?? mkTemp('zymatica-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !zymaticaHome) {
    seedConfigFrom(join(homedir(), '.zymatica'), home)
  }
  ...
  try {
    ...
    // ZYMATICA_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set ZYMATICA_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    if (devUrl) {
      env.ZYMATICA_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'zymatica-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'zymatica-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.zymatica'), home)
