...
How to build Zymatica Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Zymatica surface can also
...
- A **runtime home change** (switching the underlying `ZYMATICA_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Zymatica — do not assume
