...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'zymatica:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.zymaticaDesktop?.saveImageFromUrl) {
        if (await window.zymaticaDesktop.saveImageFromUrl(src)) {
