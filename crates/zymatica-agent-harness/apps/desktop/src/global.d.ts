...
declare global {
  interface Window {
    zymaticaDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ZymaticaConnection>
      ...
      // Zymatica Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new ZYMATICA_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ZymaticaApiRequest) => Promise<T>
      notify: (payload: ZymaticaNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<ZymaticaReadFileTextResult>
      selectPaths: (options?: ZymaticaSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ZymaticaPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ZymaticaPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: ZymaticaTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ZymaticaReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ZymaticaGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ZymaticaGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ZymaticaGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ZymaticaRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ZymaticaReviewScope, baseRef?: null | string) => Promise<ZymaticaReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ZymaticaReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ZymaticaReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ZymaticaTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ZymaticaTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ZymaticaWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ZymaticaPreviewFileChanged) => void) => () => void
...
export interface ZymaticaTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ZymaticaTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `zymatica update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  zymaticaRoot?: string
...
export interface ZymaticaConnection {
  baseUrl: string
...
export interface ZymaticaTitleBarTheme {
  background: string
  foreground: string
}

export interface ZymaticaWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Zymatica Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Zymatica Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Zymatica Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the zymatica gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Zymatica Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface ZymaticaApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface ZymaticaNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface ZymaticaPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ZymaticaReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ZymaticaPreviewWatch {
  id: string
  path: string
}
...
export interface ZymaticaGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ZymaticaGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface ZymaticaGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ZymaticaRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ZymaticaRepoStatus {
  branch: null | string
...
// turn began.
export type ZymaticaReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ZymaticaReviewFile {
  path: string
  added: number
  removed: number
...
export interface ZymaticaReviewList {
  files: ZymaticaReviewFile[]
...
export interface ZymaticaReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ZymaticaReviewShipInfo {
  ghReady: boolean
  pr: ZymaticaReviewPr | null
}

export interface ZymaticaReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ZymaticaReadDirResult {
  entries: ZymaticaReadDirEntry[]
  error?: string
}

export interface ZymaticaPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ZymaticaSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
