import type {
  ZymaticaGitBaseBranch,
  ZymaticaGitBranch,
  ZymaticaGitWorktree,
  ZymaticaRepoStatus,
  ZymaticaReviewList,
  ZymaticaReviewShipInfo
} from '@/global'
...
// (window.zymaticaDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['zymaticaDesktop']>['git']>

function desktopApi<T>(path: string, body?: Record<string, unknown>): Promise<T> {
  const desktop = window.zymaticaDesktop

  if (!desktop) {
    throw new Error('Zymatica Desktop bridge is unavailable')
  }
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: ZymaticaGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  branchList: async repoPath =>
    (await gitGet<{ branches: ZymaticaGitBranch[] }>('branches', { path: repoPath })).branches,

  baseBranchList: async repoPath =>
    (await gitGet<{ branches: ZymaticaGitBaseBranch[] }>('base-branches', { path: repoPath })).branches,

  repoStatus: repoPath => gitGet<ZymaticaRepoStatus | null>('status', { path: repoPath }),
  ...
  review: {
    list: (repoPath, scope, baseRef) =>
      gitGet<ZymaticaReviewList>('review/list', { base: baseRef, path: repoPath, scope }),
    ...
    shipInfo: repoPath => gitGet<ZymaticaReviewShipInfo>('review/ship-info', { path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  return isDesktopFsRemoteMode() ? remoteGit : window.zymaticaDesktop?.git
}
