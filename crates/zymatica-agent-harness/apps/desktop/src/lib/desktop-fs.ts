import type {
  ZymaticaConnection,
  ZymaticaReadDirResult,
  ZymaticaReadFileTextResult,
  ZymaticaSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: ZymaticaConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<ZymaticaReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<ZymaticaReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<ZymaticaReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<ZymaticaReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: ZymaticaSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
