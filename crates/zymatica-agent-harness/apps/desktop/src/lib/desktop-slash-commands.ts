...
/** Why a known Zymatica command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Zymatica profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Zymatica Agent version', surface: exec() },
...
function isKnownZymaticaSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Zymatica' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownZymaticaSlashCommand(normalized)
}
