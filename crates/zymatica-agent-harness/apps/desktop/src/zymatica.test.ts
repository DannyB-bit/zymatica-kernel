...
describe('Zymatica REST session helpers', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks()
    Reflect.deleteProperty(window, 'zymaticaDesktop')
  })
  ...
  it('gives the whole startup data burst the long timeout, not just profiles', async () => {
    ...
    const bootCalls: [() => Promise<unknown>, string][] = [
      [getZymaticaConfig, '/api/config'],
      [getZymaticaConfigDefaults, '/api/config/defaults'],
