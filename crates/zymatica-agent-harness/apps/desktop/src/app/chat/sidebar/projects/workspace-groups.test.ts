...
import type { ZymaticaGitWorktree } from '@/global'
import type { ProjectInfo, SessionInfo } from '@/types/zymatica'
...
describe('baseName', () => {
  it('returns the final path segment, ignoring trailing slashes and separators', () => {
    expect(baseName('/www/zymatica-agent/')).toBe('zymatica-agent')
...
describe('mergeRepoWorktreeGroups (visual enhancer)', () => {
  it('injects a linked worktree lane discovered by git that has no sessions yet', () => {
    ...
    const discovered: ZymaticaGitWorktree[] = [
  ...
  it('surfaces a user-named "New worktree" under .worktrees/ as its own lane', () => {
    const discovered: ZymaticaGitWorktree[] = [
      {
        branch: 'zymatica/test-gui-stuff',
        detached: false,
        isMain: false,
        locked: false,
    ...
    expect(merged.map(g => g.label)).toContain('zymatica/test-gui-stuff')
  })

  it('relabels a dir-named linked worktree lane to its live checked-out branch', () => {
    // Backend labels the lane by the worktree dir (`zymatica-agent-ci`); the live
...
describe('overlayLiveLanes', () => {
  ...
  it('places a session into an out-of-tree (sibling) worktree lane by its path', () => {
    // `zymatica-agent-ci` is a linked worktree living BESIDE the repo, not under
