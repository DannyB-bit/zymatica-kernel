...
import type { ZymaticaGitBranch } from '@/global'
...
const branchActionLabel = (branch: ZymaticaGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<ZymaticaGitBranch[]>([])
  ...
  const convert = async (branch: ZymaticaGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
