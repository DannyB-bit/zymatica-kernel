...
// bug that drives the REAL useGatewayBoot hook + REAL ZymaticaGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshZymaticaConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { zymaticaDesktop?: unknown }).zymaticaDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { zymaticaDesktop?: unknown }).zymaticaDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForZymatica) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startZymatica()'s remote branch awaits waitForZymatica() for 45s before it
    ...
    ;(window as { zymaticaDesktop?: unknown }).zymaticaDesktop = desktop
    ...
    // After ~45s waitForZymatica gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Zymatica backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
