...
import type { ZymaticaGateway } from '@/zymatica'
...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<ZymaticaGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.zymaticaDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ZymaticaGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as ZymaticaGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}, timeoutMs?: number, signal?: AbortSignal) => {
      ...
      if (!gateway) {
        throw new Error('Zymatica gateway unavailable')
      }
