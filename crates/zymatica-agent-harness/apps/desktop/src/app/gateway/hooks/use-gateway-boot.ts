import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@zymatica/shared'
import { useEffect, useRef } from 'react'

import type { ZymaticaConnection } from '@/global'
import { ZymaticaGateway } from '@/zymatica'
...
import type { RpcEvent } from '@/types/zymatica'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.zymaticaDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ZymaticaGateway | null) => void
  refreshZymaticaConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ZymaticaConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Zymatica...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Zymatica…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Zymatica gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshZymaticaConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshZymaticaConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startZymatica re-emits progress — ignore so the
    ...
    const gateway = new ZymaticaGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshZymaticaConfig(),
