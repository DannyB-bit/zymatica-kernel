...
import type { ZymaticaReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<ZymaticaReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { zymaticaDesktop: { readDir: typeof readDir } }).zymaticaDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ZymaticaReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { zymaticaDesktop: unknown }).zymaticaDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ZymaticaReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ZymaticaReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { zymaticaDesktop: unknown }).zymaticaDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.zymaticaDesktop is missing', async () => {
    delete (window as unknown as { zymaticaDesktop?: unknown }).zymaticaDesktop
