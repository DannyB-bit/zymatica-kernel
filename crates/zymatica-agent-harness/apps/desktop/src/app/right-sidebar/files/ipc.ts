...
import type { ZymaticaReadDirEntry, ZymaticaReadDirResult } from '@/global'
...
export type ProjectTreeEntry = ZymaticaReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ZymaticaReadDirEntry) {
...
async function filterIgnored(entries: ZymaticaReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ZymaticaReadDirResult> {
  if (!window.zymaticaDesktop) {
