...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `zymatica auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'nous' ? 'Zymatica Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Zymatica Portal' })
    await act(async () => {
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByText('Zymatica Portal'))
    })
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          cli_command: 'zymatica auth add qwen-oauth',
          disconnect_hint: "Use `zymatica auth add qwen-oauth` or that provider's CLI to remove it.",
