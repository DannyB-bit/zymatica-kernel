...
import { invalidateZymaticaConfig, setZymaticaConfigCache, useZymaticaConfigRecord } from '../hooks/use-config-record'
...
// Zymatica's reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Zymatica default (medium), shown as Medium.
...
// backend now surfaces the full `zymatica model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in zymatica_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useZymaticaConfigRecord()
  const setConfig = setZymaticaConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `zymatica model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Zymatica runs the flow for you.`}
          </p>
        )}
