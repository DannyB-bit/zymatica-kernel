 ...
 * log inline — the GUI equivalent of the install step `zymatica tools` runs
...
/**
 * Backend model catalog — the GUI counterpart of the model picker `zymatica
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange }: ToolsetConfigPanelProps) {
  ...
  // Guard the Zymatica Portal sign-in poll loop against unmount/state updates.
  ...
  // panel highlighted the first keyless provider (e.g. Zymatica Portal) even when
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_nous_auth) {
        ...
        // sign-in action). Reuses the existing Zymatica Portal device-code flow.
        notify({
          kind: 'warning',
  ...
  // Drive the existing Zymatica Portal OAuth device-code flow (the same session
