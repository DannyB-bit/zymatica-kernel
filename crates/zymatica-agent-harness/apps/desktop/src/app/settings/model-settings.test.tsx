...
const getZymaticaConfigRecord = vi.fn()
const saveZymaticaConfig = vi.fn()
...
vi.mock('@/zymatica', () => ({
  ...
  getZymaticaConfigRecord: () => getZymaticaConfigRecord(),
  saveZymaticaConfig: (config: unknown) => saveZymaticaConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'zymatica-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['zymatica-4', 'zymatica-4-mini'],
        authenticated: true,
        capabilities: { 'zymatica-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'zymatica-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ provider: 'nous', model: 'zymatica-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'nous', model: 'zymatica-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getZymaticaConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveZymaticaConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'nous', model: 'zymatica-4' })
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getZymaticaConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveZymaticaConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['zymatica-4'],
          authenticated: true,
          capabilities: { 'zymatica-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'zymatica-4' }]
    })
...
describe('ModelSettings MoA preset editor', () => {
  const moaConfig = () => ({
    ...
    presets: {
      default: {
        reference_models: [
          { provider: 'nous', model: 'zymatica-4' },
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('nous · zymatica-4')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
