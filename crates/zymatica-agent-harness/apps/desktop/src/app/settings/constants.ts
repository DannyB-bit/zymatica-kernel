...
export const EMPTY_SELECT_VALUE = '__zymatica_empty__'
export const CONTROL_TEXT = 'text-xs'

export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Zymatica Portal',
    description: 'Hosted Zymatica & Nous-trained models',
    docsUrl: 'https://portal.zymatica.space',
    priority: 0
  },
  {
    prefix: 'FIREWORKS_',
  ...
  { prefix: 'ZYMATICA_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // zymatica_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Zymatica needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Zymatica stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Zymatica can read from one file request.',
  approvals: {
    mode: 'How Zymatica handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Zymatica updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
