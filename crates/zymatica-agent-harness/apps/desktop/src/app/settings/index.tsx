...
import { getZymaticaConfigDefaults, getZymaticaConfigRecord, saveZymaticaConfig } from '@/zymatica'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getZymaticaConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'zymatica-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveZymaticaConfig(await getZymaticaConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
