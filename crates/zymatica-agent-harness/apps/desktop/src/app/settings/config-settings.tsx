...
import { getElevenLabsVoices, getZymaticaConfigSchema, saveZymaticaConfig } from '@/zymatica'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, ZymaticaConfigRecord } from '@/types/zymatica'

import { setZymaticaConfigCache, useZymaticaConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: ZymaticaConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ZymaticaConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useZymaticaConfigRecord()
  ...
  const {
    ...
    queryKey: ['zymatica-config-schema'],
    queryFn: getZymaticaConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: ZymaticaConfigRecord) => {
