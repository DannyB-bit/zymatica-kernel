...
import { disconnectOAuthProvider, listOAuthProviders } from '@/zymatica'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/zymatica'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.zymaticaDesktop?.terminal)
...
//      in zymatica_cli/provider_catalog.py) — the SAME provider identity
//      `zymatica model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Zymatica can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Zymatica can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Zymatica never deletes creds another tool owns behind a silent API call.
