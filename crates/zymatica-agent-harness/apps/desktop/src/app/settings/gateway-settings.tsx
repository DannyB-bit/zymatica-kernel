...
// Zymatica Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  // --- Zymatica Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.zymaticaDesktop
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.zymaticaDesktop.applyConnectionConfig(payload())
        : await window.zymaticaDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.zymaticaDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.zymaticaDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.zymaticaDesktop.getConnectionConfig(scope)
  ...
  const signOut = async () => {
    ...
    try {
      await window.zymaticaDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.zymaticaDesktop.getConnectionConfig(scope)
      acceptSavedConfig(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.zymaticaDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.zymaticaDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Zymatica Cloud panel: one portal sign-in, then a discovered-agent picker
                ...
                {cloudDiscover === 'loading' ? (
                  ...
                  <div className="flex items-start gap-2 py-3 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                    ...
                    <span>
                      ...
                      <ExternalLink href="https://portal.zymatica.space/agents" showExternalIcon={false}>
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/zymatica"
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.zymaticaDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
