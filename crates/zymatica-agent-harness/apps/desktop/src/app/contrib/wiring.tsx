...
import { getSessionMessages, triggerCronJob } from '@/zymatica'
...
import { useZymaticaConfig } from '../session/hooks/use-zymatica-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshZymaticaConfig, sttEnabled, voiceMaxRecordingSeconds } = useZymaticaConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    ...
    window.addEventListener('zymatica:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('zymatica:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshZymaticaConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshZymaticaConfig])
