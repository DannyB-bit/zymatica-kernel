...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.zymaticaDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.zymaticaDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.zymaticaDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.zymaticaDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // zymatica:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
    const unsubscribe = window.zymaticaDesktop?.onDeepLink?.(payload => {
    ...
    void window.zymaticaDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.zymaticaDesktop?.onClosePreviewRequested?.(() => void closeActiveTab())
