...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshZymaticaConfig — two REST calls (config + defaults) per event, per
    ...
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        if (apply) {
          ...
          // everything refreshZymaticaConfig applies is either active-session
        ...
        const errorMessage = payload?.message || 'Zymatica reported an error'
