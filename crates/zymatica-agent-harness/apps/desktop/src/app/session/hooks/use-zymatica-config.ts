...
import { getZymaticaConfig, getZymaticaConfigDefaults } from '@/zymatica'
...
interface ZymaticaConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useZymaticaConfig({ activeSessionIdRef, refreshProjectBranch }: ZymaticaConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshZymaticaConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getZymaticaConfig(), getZymaticaConfigDefaults().catch(() => ({}))])
  ...
  return { refreshZymaticaConfig, sttEnabled, voiceMaxRecordingSeconds }
}
