...
import { getZymaticaConfig } from '@/zymatica'
...
import { useZymaticaConfig } from './use-zymatica-config'

vi.mock('@/zymatica', () => ({
  getZymaticaConfig: vi.fn(),
  getZymaticaConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'zymatica.desktop.workspace-cwd'
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getZymaticaConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getZymaticaConfig>>)

describe('useZymaticaConfig refreshZymaticaConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshZymaticaConfig()
    })
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getZymaticaConfig>>>()
    vi.mocked(getZymaticaConfig).mockReturnValueOnce(profileConfig.promise)
    ...
    act(() => {
      pendingRefresh = result.current.refreshZymaticaConfig(true)
    })
    expect(getZymaticaConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getZymaticaConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getZymaticaConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getZymaticaConfig>>>()
    vi.mocked(getZymaticaConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshZymaticaConfig(true)
      refreshC = result.current.refreshZymaticaConfig(true)
    })
