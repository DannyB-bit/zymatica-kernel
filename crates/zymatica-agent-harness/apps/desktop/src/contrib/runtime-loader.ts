 ...
 *   (`@zymatica/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default ZymaticaPlugin -> register(ctx)
 *
 ...
 * and `<zymatica home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { getStatus } from '@/zymatica'
import { installPluginSdk, sdkImportMap } from '@/sdk/runtime'
import { notifyError } from '@/store/notifications'

import { createPluginContext, type ZymaticaPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@zymatica/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @zymatica/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: ZymaticaPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default ZymaticaPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<zymatica home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    const { zymatica_home } = await getStatus()
    const { entries } = await desktop.readDir(`${zymatica_home}/desktop-plugins`)
