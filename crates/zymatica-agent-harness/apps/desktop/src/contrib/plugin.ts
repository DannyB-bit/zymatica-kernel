 ...
 * `ZymaticaPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/zymatica'
...
export type { PluginRestOptions } from '@/zymatica'
 ...
 *  under `zymatica.plugin.<id>.` — plugins can't read or clobber each other. */
...
export interface ZymaticaPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `zymatica.plugin.${pluginId}.${key}`
