 ...
 *    `ZymaticaPlugin` registers automatically (vite glob — drop a folder in).
 *    None ship in-tree today; reference/demo plugins live in the companion
 *    `zymatica-example-plugins` repo.
 *  - RUNTIME: the on-disk door (`<zymatica home>/desktop-plugins/<name>/plugin.js`)
...
const modules = import.meta.glob<{ default: ZymaticaPlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default ZymaticaPlugin export — skipped`)
