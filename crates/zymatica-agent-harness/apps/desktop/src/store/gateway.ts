import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@zymatica/shared'
...
const isOpen = (gateway: ZymaticaGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<ZymaticaGateway | null>(null)
...
let primaryGateway: ZymaticaGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ZymaticaGateway | null, profile = 'default'): void {
...
export function activeGateway(): ZymaticaGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<ZymaticaGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
