...
import { getZymaticaConfigRecord, saveZymaticaConfig } from '@/zymatica'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getZymaticaConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveZymaticaConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
