...
const { getProfiles } = await import('@/zymatica')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/zymatica/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<ZymaticaConnection> = {}): ZymaticaConnection =>
  ({ baseUrl: 'https://zymatica-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ZymaticaConnection

const localConn = (over: Partial<ZymaticaConnection> = {}): ZymaticaConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ZymaticaConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ZymaticaConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { zymaticaDesktop: { getConnection } })
