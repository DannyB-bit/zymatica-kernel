...
const STORAGE_KEY = 'zymatica:native-notifications'
...
// "Backgrounded" = the user isn't on Zymatica. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.zymaticaDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.zymaticaDesktop
