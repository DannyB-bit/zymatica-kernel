...
import type { ZymaticaReviewFile, ZymaticaReviewShipInfo } from '@/global'
...
// reaches the gateway); everything else routes through window.zymaticaDesktop.git,
...
function file(path: string, over: Partial<ZymaticaReviewFile> = {}): ZymaticaReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as ZymaticaReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.zymaticaDesktop. Any op not supplied defaults
...
function stubReview(over: ReviewStub = {}) {
  ...
  ;(window as unknown as { zymaticaDesktop?: unknown }).zymaticaDesktop = {
    git: { review },
    openExternal: vi.fn()
  }
...
afterEach(() => {
  delete (window as unknown as { zymaticaDesktop?: unknown }).zymaticaDesktop
})
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as ZymaticaReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.zymaticaDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: ZymaticaReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as ZymaticaReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { zymaticaDesktop?: unknown }).zymaticaDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as ZymaticaReviewShipInfo)
