...
import type { SessionInfo, UsageStats } from '@/types/zymatica'
...
const COMPOSER_MODEL_KEY = 'zymatica.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'zymatica.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'zymatica.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'zymatica.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'zymatica.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'zymatica.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'zymatica.desktop.lastRoute'
...
function workspaceCwdKey(connection: ZymaticaConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.zymaticaDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.zymaticaDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<ZymaticaConnection | null>(null)
...
export const setConnection = (next: Updater<ZymaticaConnection | null>) => updateAtom($connection, next)
