...
export function setZoomPercent(percent: number): void {
  window.zymaticaDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.zymaticaDesktop?.zoom) {
  void window.zymaticaDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.zymaticaDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
