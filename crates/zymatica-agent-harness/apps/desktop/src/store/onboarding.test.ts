...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('zymatica-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('zymatica-desktop-onboarded-v1')).toBe('1')
  })
...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: provider('nous', 'Zymatica Portal'),
          start: {
        ...
        reason:
          'No access token found for Zymatica Portal login. setup.status reports configured credentials, but runtime resolution still failed.',
        requested: true
      })
    )
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('Zymatica Portal')
