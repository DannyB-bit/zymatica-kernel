...
import type { ZymaticaGitBaseBranch, ZymaticaGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/zymatica'
...
const PROJECT_SCOPE_KEY = 'zymatica.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Zymatica gateway is not connected')
  }
...
// repo, returning where Zymatica should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ZymaticaGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<ZymaticaGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.zymaticaDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.zymaticaDesktop?.writeClipboard?.(path)
  }
}
