...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.zymaticaDesktop?.openSessionWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await openWindow(() => window.zymaticaDesktop.openSessionWindow(sessionId, opts), 'Could not open chat in a new window')
}

// Open a fresh compact window on the new-session draft.
export async function openNewSessionInNewWindow(): Promise<void> {
  if (!canOpenSessionWindow() || typeof window.zymaticaDesktop.openNewSessionWindow !== 'function') {
    return
  }

  await openWindow(() => window.zymaticaDesktop.openNewSessionWindow(), 'Could not open new session window')
}
