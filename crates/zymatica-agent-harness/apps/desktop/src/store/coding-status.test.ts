...
import type { ZymaticaRepoStatus } from '@/global'

import { $repoStatus, $repoStatusLoading, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: ZymaticaRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<ZymaticaRepoStatus | null>) {
  ;(window as unknown as { zymaticaDesktop?: unknown }).zymaticaDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh', async () => {
    const resolvers: Array<(status: ZymaticaRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
