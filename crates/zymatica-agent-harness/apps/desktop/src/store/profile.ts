...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/zymatica'
...
import type { ProfileInfo } from '@/types/zymatica'
...
// /api/profiles/active `current`). "default" is the root ~/.zymatica. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'zymatica.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'zymatica.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.zymaticaDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ZYMATICA_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.zymaticaDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.zymaticaDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'zymatica.desktop.showAllProfiles'
...
// Switch to the default (root ~/.zymatica) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.zymaticaDesktop?.touchBackend?.(target).catch(() => undefined)
}
