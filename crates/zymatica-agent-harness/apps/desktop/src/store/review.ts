...
//
// Scope is always "uncommitted" — Zymatica' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'zymatica.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'zymatica.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'zymatica.desktop.reviewTreeMode'
const SELECTED_KEY = 'zymatica.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ZymaticaReviewFile[]>([])
...
export const $reviewShipInfo = atom<ZymaticaReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['zymaticaDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ZymaticaReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.zymaticaDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.zymaticaDesktop?.openExternal?.(url)
    }
