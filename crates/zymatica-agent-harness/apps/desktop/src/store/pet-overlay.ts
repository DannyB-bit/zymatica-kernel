 ...
 * app's bounds and stays visible while Zymatica is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'zymatica.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'zymatica.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.zymaticaDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.zymaticaDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.zymaticaDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.zymaticaDesktop?.petOverlay?.close()
}
