...
import { getZymaticaConfigRecord, type ZymaticaConfigRecord, saveZymaticaConfig } from '@/zymatica'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ZymaticaConfigRecord>
  saveConfig: (config: ZymaticaConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.zymaticaDesktop?.api) {
      return Promise.resolve({})
    }

    return getZymaticaConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.zymaticaDesktop?.api) {
    ...
    return saveZymaticaConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ZymaticaConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ZymaticaConfigRecord, locale: Locale): ZymaticaConfigRecord {
