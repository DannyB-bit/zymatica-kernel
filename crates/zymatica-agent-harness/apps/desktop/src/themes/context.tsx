...
const SKIN_KEY = 'zymatica-desktop-theme-v2'
const MODE_KEY = 'zymatica-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'zymatica-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'zymatica-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'zymatica-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.zymaticaTheme = skinName
  root.dataset.zymaticaMode = rendered
  ...
  window.zymaticaDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('zymatica-boot-background', chromeBg)
    window.localStorage.setItem('zymatica-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.zymaticaThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.zymaticaDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Zymatica backend theme on connect. */
