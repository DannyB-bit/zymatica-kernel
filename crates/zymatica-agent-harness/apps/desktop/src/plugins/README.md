...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `ZymaticaPlugin` and
...
[`zymatica-example-plugins`](https://github.com/Zymatica.space/zymatica-example-plugins)
repo so the shipped app stays uncluttered.

User- and agent-authored plugins load at runtime from
`$ZYMATICA_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
