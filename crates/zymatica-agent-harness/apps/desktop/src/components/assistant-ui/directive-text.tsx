...
const ZYMATICA_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type ZymaticaRefType = (typeof ZYMATICA_REF_TYPES)[number]
...
const ICON_PATHS: Record<ZymaticaRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ZymaticaRefType] ?? ICON_FALLBACK
...
const ZYMATICA_DIRECTIVE_RE = new RegExp(
...
export const zymaticaDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ZYMATICA_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ZymaticaRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ZymaticaRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...zymaticaDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Zymatica directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.zymaticaDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.zymaticaDesktop?.readFileDataUrl(id)
