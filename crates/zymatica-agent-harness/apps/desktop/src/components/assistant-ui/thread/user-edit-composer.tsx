...
import { zymaticaDirectiveFormatter } from '@/components/assistant-ui/directive-text'
import {
...
export const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const replaceTriggerWithChip = useCallback(
    (item: Unstable_TriggerItem) => {
      ...
      rememberInitialDraft()
      const serialized = zymaticaDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLElement>) => {
    if (!dragHasAttachments(event.dataTransfer, ZYMATICA_PATHS_MIME)) {
      return
    }
