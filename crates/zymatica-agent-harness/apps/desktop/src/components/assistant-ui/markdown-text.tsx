...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.zymaticaDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.zymaticaDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.zymaticaDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.zymaticaDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.zymaticaDesktop && text && normalizeExternalUrl(text) === target) {
