...
import { formatRefValue, zymaticaDirectiveFormatter } from './directive-text'
...
describe('zymaticaDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = zymaticaDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = zymaticaDirectiveFormatter.parse('@file:src/main.tsx the entry point')
