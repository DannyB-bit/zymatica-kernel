...
const STORAGE_KEY = 'zymatica.desktop.layoutTree.v2'

writeKey('zymatica.desktop.layoutTree.v1', null)
 ...
 */
export const $activePresetId = atom<string>(readKey('zymatica.desktop.layoutPreset.active') ?? 'default')

export function markActivePreset(id: string) {
  $activePresetId.set(id)
  writeKey('zymatica.desktop.layoutPreset.active', id)
}
 ...
 */
const DISMISSED_KEY = 'zymatica.desktop.dismissedPanes.v1'
...
const USER_PLACED_KEY = 'zymatica.desktop.userPlacedPanes.v1'
...
if (import.meta.env.DEV && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__ZYMATICA_LAYOUT_TREE__ = {
    close: closeTreePane,
