...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Zymatica gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.zymaticaDesktop
  Object.defineProperty(window, 'zymaticaDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'zymaticaDesktop', { configurable: true, value: original })
}
