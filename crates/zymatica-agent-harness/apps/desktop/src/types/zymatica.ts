...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Zymatica doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in zymatica_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `zymatica model` uses — instead of
...
export interface ZymaticaConfig {
  agent?: {
...
export type ZymaticaConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `zymatica model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors zymatica_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Zymatica, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `zymatica tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Zymatica); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/zymatica/update/check` — the backend's own update state.
