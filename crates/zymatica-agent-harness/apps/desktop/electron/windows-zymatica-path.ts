/**
 * windows-zymatica-path.ts
 *
 * Pure, dependency-injected pieces of Windows `zymatica` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvZymaticaCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `zymatica` shim shadowed the real
 *      zymatica.cmd/zymatica.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\zymatica.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvZymaticaCommand() — unwrapWindowsVenvZymaticaCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `zymatica`) must
 * not shadow `zymatica.cmd`/`zymatica.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv zymatica shim, bootstrap-complete
 * marker) — gating on just the zymatica.exe console-script shim alone is the
...
export interface ResolveVenvZymaticaCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportZymaticaCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `zymatica`/`zymatica.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/zymatica(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m zymatica_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportZymaticaCli(). A
 * venv whose update died mid-`pip install` still has python.exe + zymatica.exe
 ...
 *
 * Returns null when `command` is not a venv zymatica shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvZymaticaCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvZymaticaCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^zymatica(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Zymatica at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Zymatica Python at ${python}`,
    command: python,
