...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'zymatica-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/zymatica-runner-test',
    sourceRepoRoot: null,
    zymaticaHome: '/tmp/zymatica-runner-test',
    logRoot: '/tmp/zymatica-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'zymatica-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'zymatica-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/zymatica-agent',
      zymaticaHome: '/tmp/zymatica'
    }),
    ['--dir', '/tmp/zymatica-agent', '--zymatica-home', '/tmp/zymatica', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/zymatica-agent', '--zymatica-home', '/tmp/zymatica', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/zymatica',
      zymaticaHome: '/tmp/home'
    }),
    ['--dir', '/tmp/zymatica', '--zymatica-home', '/tmp/home', '--branch', 'main']
  )
})
