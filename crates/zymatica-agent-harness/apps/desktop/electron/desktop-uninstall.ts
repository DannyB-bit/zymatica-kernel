 ...
 * the three user-facing uninstall modes to the `zymatica uninstall` CLI flags,
 ...
 *              `zymatica uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `zymatica uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `zymatica uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `zymatica` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m zymatica_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `zymatica_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'zymatica_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Zymatica.app/Contents/MacOS/Zymatica  → …/Zymatica.app
 *   Windows: …\Zymatica\Zymatica.exe                 → …\Zymatica  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Zymatica.app/Contents/MacOS/Zymatica → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Zymatica.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Zymatica.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Zymatica$/i.test(dir) || /[\\/]zymatica-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/zymatica
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import zymatica_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, zymaticaHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export ZYMATICA_HOME=${q(zymaticaHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import zymatica_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Zymatica install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ZYMATICA_HOME=${String(zymaticaHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
