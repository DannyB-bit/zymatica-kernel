 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Zymatica Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.zymatica-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(zymaticaHome) {
  return path.join(zymaticaHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.zymatica/zymatica-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(zymaticaHome) {
  if (!zymaticaHome) {
    return null
  }

  const candidate = path.join(zymaticaHome, 'zymatica-agent', 'scripts', installScriptName())
...
function cachedScriptPath(zymaticaHome, commit) {
  return path.join(bootstrapCacheDir(zymaticaHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/DannyB-bit/Zymatica.space-BODY/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(zymaticaHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(zymaticaHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, zymaticaHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ZYMATICA_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ZYMATICA_HOME: zymaticaHome || process.env.ZYMATICA_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, zymaticaHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, zymaticaHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--zymatica-home', zymaticaHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, zymaticaHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, zymaticaHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, zymaticaHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(zymaticaHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, zymaticaHome, emit })
