 ...
 *     `X-Zymatica-Session-Token` header; WS uses `?token=`.
...
// zymatica_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - zymatica_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - zymatica_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / zymatica #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-zymatica_session_at', '__Secure-zymatica_session_at', 'zymatica_session_at']
const RT_COOKIE_VARIANTS = ['__Host-zymatica_session_rt', '__Secure-zymatica_session_rt', 'zymatica_session_rt']

// The Nous portal (NAS) does NOT use Zymatica gateway session cookies — it is a
...
// Zymatica Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://zymatica.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a zymatica session ACCESS-token cookie
 ...
 * the Zymatica gateway session cookies, so cloud sign-in / discovery liveness
