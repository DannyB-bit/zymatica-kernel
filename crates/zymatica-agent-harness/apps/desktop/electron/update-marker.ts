...
export function markerPath(zymaticaHome) {
  return path.join(zymaticaHome, '.zymatica-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(zymaticaHome)
  let raw
 ...
 *
 * The Tauri-based zymatica-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(zymaticaHome, pid, { now = Date.now } = {}) {
  const file = markerPath(zymaticaHome)
