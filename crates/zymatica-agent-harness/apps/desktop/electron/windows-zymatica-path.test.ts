// Unit tests for the pure Windows `zymatica` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvZymaticaCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `zymatica` shim shadows
//      the real zymatica.cmd/zymatica.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the zymatica.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvZymaticaCommand() — must probe the venv python via
//      canImportZymaticaCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvZymaticaCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportZymaticaCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    zymaticaHome: '/fake/zymatica-home',
...
test('resolveVenvZymaticaCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvZymaticaCommand('/root/venv/Scripts/zymatica.exe', [], deps), null)
})

test('resolveVenvZymaticaCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvZymaticaCommand('/root/venv/Scripts/zymatica.cmd', [], deps), null)
})

test('resolveVenvZymaticaCommand: returns null when the basename is not zymatica/zymatica.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvZymaticaCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvZymaticaCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvZymaticaCommand('/root/venv/bin/zymatica.exe', [], deps), null)
})

test('resolveVenvZymaticaCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvZymaticaCommand('/root/venv/Scripts/zymatica.exe', [], deps), null)
})

test('resolveVenvZymaticaCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportZymaticaCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvZymaticaCommand('/root/venv/Scripts/zymatica.exe', ['serve'], deps)
...
test('resolveVenvZymaticaCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvZymaticaCommand('/root/venv/Scripts/zymatica.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'zymatica_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvZymaticaCommand: is case-insensitive on zymatica.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvZymaticaCommand('/root/venv/Scripts/ZYMATICA.EXE', [], deps))
  assert.ok(resolveVenvZymaticaCommand('/root/venv/SCRIPTS/zymatica.exe', [], deps))
})
