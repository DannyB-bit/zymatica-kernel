...
import { buildDesktopBackendEnv, normalizeZymaticaHomeRoot } from './backend-env'
import { canImportZymaticaCli, verifyZymaticaCli } from './backend-probes'
...
import {
  ...
  resolveVenvZymaticaCommand
} from './windows-zymatica-path'
import {
  alreadyHasNoSandbox,
...
const USER_DATA_OVERRIDE = process.env.ZYMATICA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ZYMATICA_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ZYMATICA_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ZYMATICA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[zymatica] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[zymatica] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `zymatica desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Zymatica sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[zymatica] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[zymatica] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[zymatica] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[zymatica] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[zymatica] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('zymatica:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[zymatica] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[zymatica] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[zymatica] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[zymatica] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ZYMATICA_HOME — the user-facing root for everything Zymatica-related. Mirrors
// scripts/install.ps1's $ZymaticaHome and scripts/install.sh's $ZYMATICA_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\zymatica (matches install.ps1)
//   macOS / Linux: ~/.zymatica (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.zymatica directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\zymatica yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ZYMATICA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ZYMATICA_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.zymatica / %LOCALAPPDATA%\zymatica.
function resolveZymaticaHome() {
  if (process.env.ZYMATICA_HOME) {
    return normalizeZymaticaHomeRoot(process.env.ZYMATICA_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'zymatica-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ZYMATICA_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\zymatica and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ZYMATICA_HOME')

    if (fromRegistry) {
      return normalizeZymaticaHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'zymatica')
    const legacy = path.join(app.getPath('home'), '.zymatica')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.zymatica setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.zymatica')
}

const ZYMATICA_HOME = resolveZymaticaHome()

function zymaticaManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_zymatica_node_dirs() in
  // zymatica_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(ZYMATICA_HOME, 'node')
...
function pathWithZymaticaManagedNode(...entries) {
  return [...zymaticaManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ZYMATICA_ROOT — the canonical mutable Zymatica install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ZYMATICA_ROOT = path.join(ZYMATICA_HOME, 'zymatica-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ZYMATICA_ROOT, 'venv')
...
// boots, going straight to `resolveZymaticaBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ZYMATICA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ZYMATICA_ROOT, '.zymatica-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Zymatica profile the desktop launches its
// local backend as. When set, startZymatica() passes `zymatica --profile <name>
// dashboard …`, which deterministically pins ZYMATICA_HOME (see
// _apply_profile_override in zymatica_cli/main.py) and bypasses the sticky
// ~/.zymatica/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors zymatica_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// zymaticaDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under ZYMATICA_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by zymatica_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(ZYMATICA_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (zymatica_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ZYMATICA_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ZYMATICA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ZYMATICA_DESKTOP_APP_NAME || 'Zymatica'
...
// 'zymatica:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.zymatica.space.zymatica) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.zymatica.space.zymatica')
}

// Seed the native About panel with the live Zymatica version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `zymatica update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveZymaticaVersion(),
  copyright: 'Copyright © 2026 Zymatica Team'
})
...
const MEDIA_PROTOCOL = 'zymatica-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(zymatica-media://…)`.
...
// startZymatica(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.ZYMATICA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.ZYMATICA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startZymatica() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning zymatica serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Zymatica backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  const lines = text.split(/\r?\n/).map(line => `[zymatica] ${line}`)
  zymaticaLog.push(...lines)

  if (zymaticaLog.length > 300) {
    zymaticaLog.splice(0, zymaticaLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-zymatica-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('zymatica:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the zymatica:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('zymatica:bootstrap:event', ev)
}
...
// The Tauri updater writes ZYMATICA_HOME/.zymatica-update-in-progress for the whole
...
// (`force_kill_other_zymatica`, taskkill /IM zymatica.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(ZYMATICA_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Zymatica will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(ZYMATICA_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `zymatica` — must not shadow `zymatica.cmd`/`zymatica.exe`.
...
function unwrapWindowsVenvZymaticaCommand(command, backendArgs) {
  return resolveVenvZymaticaCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
    canImportZymaticaCli,
    getVenvPython,
    getVenvSitePackagesEntries,
    buildDesktopBackendEnv,
    zymaticaHome: ZYMATICA_HOME,
...
// spawns `zymatica serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `zymatica` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'zymatica_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, ZYMATICA_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isZymaticaSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'zymatica_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.ZYMATICA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Zymatica deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Zymatica' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\zymatica\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'zymatica', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'zymatica', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal ZYMATICA_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\zymatica\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'zymatica', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'zymatica', 'git', 'bin', 'git.exe'))
  }
...
function recentZymaticaLog() {
  return zymaticaLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's zymatica root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ZYMATICA_ROOT.
// ZYMATICA_DESKTOP_ZYMATICA_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ZYMATICA_DESKTOP_ZYMATICA_ROOT && path.resolve(process.env.ZYMATICA_DESKTOP_ZYMATICA_ROOT),
    !IS_PACKAGED && isZymaticaSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isZymaticaSourceRoot(ACTIVE_ZYMATICA_ROOT) ? ACTIVE_ZYMATICA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ZYMATICA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('zymatica:updates:progress', merged)
  }
}
...
// ZYMATICA_HOME/zymatica-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_zymatica_home). That binary owns
// ALL repo mutation — running `zymatica update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'zymatica-setup.exe' : 'zymatica-setup'
  const candidate = path.join(ZYMATICA_HOME, name)
...
// Path to the venv shim whose lock decides whether `zymatica update` can write
// fresh entry points. On Windows this is the file the running backend
// `zymatica.exe` holds open; on POSIX it's never mandatory-locked.
function venvZymaticaShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'zymatica.exe')
    : path.join(updateRoot, 'venv', 'bin', 'zymatica')
}
...
// only signals the direct child, so on Windows a backend `zymatica.exe` that
// spawned its own grandchildren (a `zymatica` REPL, a pty terminal session, the
...
// `zymaticaProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `zymatica.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a zymatica
// REPL, a pty terminal, the gateway) keep `zymatica.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const zymaticaProcess = backendConnectionState.getProcess()

  if (zymaticaProcess && Number.isInteger(zymaticaProcess.pid)) {
    pids.push(zymaticaProcess.pid)
  }
  ...
  if (zymaticaProcess && !zymaticaProcess.killed) {
    try {
      zymaticaProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvZymaticaShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentZymaticaProcess = backendConnectionState.getProcess()

    if (currentZymaticaProcess && Number.isInteger(currentZymaticaProcess.pid)) {
      stragglers.push(currentZymaticaProcess.pid)
    }
...
// `zymatica update`). Instead we spawn the staged Zymatica-Setup binary with
// --update and quit, so it can run `zymatica update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri zymatica-setup. Unlike Windows
      ...
      // whole update itself: `zymatica update` (backend) + `zymatica desktop
    ...
    if (!updater) {
      ...
      // `zymatica desktop`, never the Tauri installer that self-copies
      // zymatica-setup.exe into ZYMATICA_HOME). They DO have a working `zymatica`
      ...
      // checkout they're on — bare `zymatica update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `zymatica update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `zymatica update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, zymaticaRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Zymatica — this window will close and the updater will open. Don’t reopen Zymatica yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // zymatica.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running zymatica, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Zymatica install open ' +
        '(a second Zymatica window or a terminal running zymatica?). Close it and retry.'
    ...
    // `zymatica update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: ZYMATICA_HOME,
      env: {
        ...process.env,
        ZYMATICA_HOME,
        PATH: pathWithZymaticaManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(ZYMATICA_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvZymatica = path.join(venvBin, IS_WINDOWS ? 'zymatica.exe' : 'zymatica')
  ...
  // not just the `zymatica.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvZymatica) || fileExists(path.join(updateRoot, '.zymatica-bootstrap-complete'))
...
// Resolve the zymatica CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `zymatica` on PATH.
function resolveZymaticaCliBinary(updateRoot) {
  const venvZymatica = path.join(updateRoot, 'venv', 'bin', 'zymatica')

  if (fileExists(venvZymatica)) {
    return venvZymatica
  }

  return findOnPath('zymatica') || null
}
...
// macOS/Linux in-app update: backend (`zymatica update`) + OS-aware GUI rebuild
// (`zymatica desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const zymatica = resolveZymaticaCliBinary(updateRoot)

  if (!zymatica) {
    emitUpdateProgress({ stage: 'manual', message: 'zymatica update', percent: null })

    return { ok: true, manual: true, command: 'zymatica update', zymaticaRoot: updateRoot }
  }

  // Put the Zymatica-managed Node and the venv on PATH so `zymatica desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\zymatica\node, not node\bin.
  // PYTHONUNBUFFERED: `zymatica update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    ZYMATICA_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithZymaticaManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `zymatica update` reaps stale `zymatica serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads ZYMATICA_DESKTOP_CHILD_PID and excludes
  ...
  if (zymaticaProcess && Number.isInteger(zymaticaProcess.pid)) {
    desktopChildPids.push(zymaticaProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.ZYMATICA_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Zymatica (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(zymatica, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'zymatica update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'zymatica update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(zymatica, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Zymatica to retry.',
  ...
  // Linux in-app update terminal state (#45205). `zymatica desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Zymatica…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `zymatica-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Zymatica to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Zymatica desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Zymatica to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Zymatica.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Zymatica.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Zymatica to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.zymatica-update-new"; then
    rm -rf "$DST.zymatica-update-old" 2>/dev/null || true
    mv "$DST" "$DST.zymatica-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.zymatica-update-new" "$DST"
    rm -rf "$DST.zymatica-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Zymatica to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_ZYMATICA_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isZymaticaSourceRoot(ACTIVE_ZYMATICA_ROOT) &&
    fileExists(venvPython) &&
    canImportZymaticaCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_ZYMATICA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `zymatica
...
function resolveWebDist() {
  const override = process.env.ZYMATICA_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ZYMATICA_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: zymatica desktop --force-build`
  )
...
function resolveZymaticaCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Zymatica.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.ZYMATICA_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveZymaticaCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'zymatica_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_ZYMATICA_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Zymatica at ${ACTIVE_ZYMATICA_ROOT}`,
    command,
    args: ['-m', 'zymatica_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      zymaticaHome: ZYMATICA_HOME,
      pythonPathEntries: [ACTIVE_ZYMATICA_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ZYMATICA_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveZymaticaBackend(backendArgs) {
  // 1. Explicit override -- ZYMATICA_DESKTOP_ZYMATICA_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ZYMATICA_DESKTOP_ZYMATICA_ROOT && path.resolve(process.env.ZYMATICA_DESKTOP_ZYMATICA_ROOT)

  if (overrideRoot && isZymaticaSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Zymatica source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `zymatica` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isZymaticaSourceRoot.)
  if (!IS_PACKAGED && isZymaticaSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Zymatica source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_ZYMATICA_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\zymatica\zymatica-agent (Windows) or ~/.zymatica/zymatica-agent.
  ...
  //    to spawning zymatica. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `zymatica update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `zymatica` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    ZYMATICA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ZYMATICA_DESKTOP_IGNORE_EXISTING !== '1') {
    let zymaticaCommand = null
    const zymaticaOverride = process.env.ZYMATICA_DESKTOP_ZYMATICA

    if (zymaticaOverride) {
      const resolvedOverride = findOnPath(zymaticaOverride)

      if (resolvedOverride) {
        zymaticaCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(zymaticaOverride, { isWsl: IS_WSL })) {
        zymaticaCommand = zymaticaOverride
      } else {
        rememberLog(`Ignoring Windows Zymatica override under WSL: ${zymaticaOverride}`)
      }
    } else {
      zymaticaCommand = findOnPath('zymatica')
    }

    if (zymaticaCommand) {
      if (looksLikeDesktopAppBinary(zymaticaCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Zymatica CLI: ${zymaticaCommand}`)
        zymaticaCommand = null
      }
    }

    if (zymaticaCommand) {
      const unwrapped = unwrapWindowsVenvZymaticaCommand(zymaticaCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `zymatica` shim
      ...
      const shellForProbe = isCommandScript(zymaticaCommand)

      if (verifyZymaticaCli(zymaticaCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvZymaticaCommand(zymaticaCommand, backendArgs) || {
            label: `existing Zymatica CLI at ${zymaticaCommand}`,
            command: zymaticaCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Zymatica CLI at ${zymaticaCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed zymatica_cli module via system Python.
  ...
  if (python) {
    ...
    // having zymatica_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\zymatica\zymatica-agent\venv.
    if (canImportZymaticaCli(python)) {
      return {
        kind: 'python',
        label: `installed zymatica_cli module via ${python}`,
        command: python,
        args: ['-m', 'zymatica_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: zymatica_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveZymaticaBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Zymatica Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_ZYMATICA_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveZymaticaBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Zymatica install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Zymatica recovery was handed off to Zymatica Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      zymaticaHome: ZYMATICA_HOME,
      logRoot: path.join(ZYMATICA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Zymatica install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Zymatica bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(ZYMATICA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startZymatica() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // zymatica:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveZymaticaBackend(backend.args))
  }
  ...
  if (!isZymaticaSourceRoot(ACTIVE_ZYMATICA_ROOT)) {
    throw new Error(
      `Zymatica install at ${ACTIVE_ZYMATICA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Zymatica' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\zymatica\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `zymatica` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Zymatica on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Zymatica.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isZymaticaSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Zymatica venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Zymatica at ${ACTIVE_ZYMATICA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Zymatica runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----zymatica-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Zymatica backend URL protocol: ${parsed.protocol}`))
    ...
    const req = client.request(
      parsed,
      {
        ...
        headers: {
          ...
          'X-Zymatica-Session-Token': token,
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Zymatica backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Zymatica backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Zymatica-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('zymatica:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveZymaticaCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('zymatica:preview-file-changed', payload)
}
...
async function waitForZymatica(baseUrl, token) {
  ...
  throw new Error(`Zymatica backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('zymatica:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('zymatica:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Zymatica...".
function sendPowerResume() {
  ...
  webContents.send('zymatica:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('zymatica:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('zymatica:window-state-changed', state)
}
...
//
// Hosted Zymatica gateways gate the dashboard behind an OAuth provider (e.g.
// Zymatica Team) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``zymatica_session_at``),
...
//   * Zymatica Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / zymatica #37247).
//     Both are set as HttpOnly cookies (``zymatica_session_at`` ~15 min,
//     ``zymatica_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:zymatica-remote-oauth'
...
// "not signed in", which on the initial boot path (startZymatica → the renderer's
// single-shot boot() with no retry) surfaced as the "Zymatica couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Zymatica
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Zymatica Cloud agent…' : 'Sign in to Zymatica gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Zymatica backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Zymatica Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:zymatica-remote-oauth`:
...
// convention (zymatica_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Zymatica surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.zymatica.space'

function resolvePortalBaseUrl() {
  const raw = process.env.ZYMATICA_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Zymatica gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for zymatica_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Zymatica Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Zymatica Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// Discover the hosted (Zymatica Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Zymatica Cloud. Open Settings → Gateway, choose Zymatica Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Zymatica Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Zymatica Cloud session has expired. Sign in to Zymatica Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Zymatica Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root ZYMATICA_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ZYMATICA_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.ZYMATICA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  // The env override forces a plain remote connection. Otherwise reflect the
  // saved mode, preserving 'cloud' (a Zymatica Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted Zymatica Cloud org (slug/id) for a cloud connection, or '' for
    ...
    // scope is never overridden by ZYMATICA_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Zymatica Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Zymatica Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (zymatica #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Zymatica gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Zymatica gateway is selected, but no session token is saved. ' +
...
//   2. env override (ZYMATICA_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.ZYMATICA_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.ZYMATICA_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'ZYMATICA_DESKTOP_REMOTE_URL is set but ZYMATICA_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Zymatica backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.ZYMATICA_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startZymatica() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Zymatica
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Zymatica Team" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startZymatica())
  ...
  // connect to Zymatica gateway". Mirror the renderer's connect here so the test
...
function resetZymaticaConnection({ soft = false } = {}) {
  backendStartFailure = null
  const zymaticaProcess = backendConnectionState.invalidate()
  stopBackendChild(zymaticaProcess)
...
// startZymatica() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetZymaticaConnection() invalidates it.
  const zymaticaProcess = backendConnectionState.getProcess()
  const dying = zymaticaProcess && !zymaticaProcess.killed ? zymaticaProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('zymatica:connection:applied')
}
...
// profile to startZymatica() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startZymatica() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForZymatica(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: zymaticaLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited ZYMATICA_HOME env (see _apply_profile_override
  // step 3 in zymatica_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveZymaticaBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const zymaticaCwd = resolveZymaticaCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Zymatica backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: zymaticaCwd,
        ZYMATICA_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        ZYMATICA_DESKTOP: '1',
        ZYMATICA_WEB_DIST: webDist,
        ...(readyFile ? { ZYMATICA_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Zymatica backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Zymatica backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Zymatica backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForZymatica(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Zymatica backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_zymatica_home() recreates the deleted profile directory.
//
...
async function startZymatica() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startZymatica() call re-throws the same error
  ...
  const connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Zymatica backend', 8)
    ...
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Zymatica backend at ${remote.baseUrl}`, 24)
      await waitForZymatica(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Zymatica backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.zymatica/active_profile file) and
    // resolves ZYMATICA_HOME the same way `zymatica -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Zymatica runtime', 28)
    const backend = await ensureRuntime(resolveZymaticaBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const zymaticaCwd = resolveZymaticaCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Zymatica backend via ${backend.label}`, 84)
    rememberLog(`Starting Zymatica backend via ${backend.label}`)

    const zymaticaProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: zymaticaCwd,
        env: {
          ...process.env,
          // Explicitly pin ZYMATICA_HOME for the child so Python's get_zymatica_home()
          // resolves to the SAME location our resolveZymaticaHome() picked. Without
          // this pin, Python falls back to ~/.zymatica on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\zymatica, which differs from C:\Users\<u>\.zymatica.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets ZYMATICA_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, zymaticaProcess)

    if (!processOwner) {
      stopBackendChild(zymaticaProcess)
      throw new Error('Zymatica backend start was superseded by a newer connection attempt.')
    }

    zymaticaProcess.stdout.on('data', rememberLog)
    zymaticaProcess.stderr.on('data', rememberLog)
    ...
    zymaticaProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Zymatica backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Zymatica backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Zymatica backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Zymatica backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    zymaticaProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Zymatica backend exit (${signal || code})`)
      ...
      rememberLog(`Zymatica backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Zymatica backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Zymatica backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentZymaticaLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Zymatica backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(zymaticaProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Zymatica backend to become ready', 90)
    await Promise.race([waitForZymatica(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => zymaticaProcess.exitCode === null && !zymaticaProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Zymatica backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Zymatica is
...
// pushes pet state over IPC (zymatica:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via zymatica:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Zymatica app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Zymatica icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see zymatica:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Zymatica in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('zymatica:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  startZymatica().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('zymatica:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Zymatica…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('zymatica:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetZymaticaConnection only
    // clears the connection promise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Zymatica backend failed liveness probe; dropping stale connection.')
    resetZymaticaConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('zymatica:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('zymatica:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('zymatica:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('zymatica:window:openNewSession', async () => {
...
ipcMain.handle('zymatica:zoom:get', event => {
...
ipcMain.on('zymatica:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('zymatica:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('zymatica:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('zymatica:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('zymatica:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('zymatica:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('zymatica:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('zymatica:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('zymatica:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('zymatica:pet-overlay:control', payload)
})
ipcMain.handle('zymatica:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startZymatica() call restarts the
...
ipcMain.handle('zymatica:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startZymatica() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('zymatica:bootstrap:cancel', async () => {
...
ipcMain.handle('zymatica:boot-progress:get', async () => bootProgressState)
ipcMain.handle('zymatica:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('zymatica:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('zymatica:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('zymatica:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('zymatica:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('zymatica:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Zymatica Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('zymatica:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('zymatica:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('zymatica:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on zymatica:connection:applied.
...
ipcMain.handle('zymatica:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('zymatica:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ZYMATICA_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('zymatica:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('zymatica:requestMicrophoneAccess', async () => {
...
ipcMain.handle('zymatica:api', async (_event, request) => {
  ...
  // backend calls ensure_zymatica_home() which recreates the profile directory,
...
ipcMain.handle('zymatica:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('zymatica:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('zymatica:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('zymatica:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('zymatica:readFileText', async (_event, filePath) => {
...
ipcMain.handle('zymatica:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('zymatica:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('zymatica:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('zymatica:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('zymatica:saveClipboardImage', async () => {
...
ipcMain.handle('zymatica:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('zymatica:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('zymatica:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('zymatica:titlebar-theme', (_event, payload) => {
...
ipcMain.on('zymatica:native-theme', (_event, mode) => {
...
ipcMain.on('zymatica:translucency', (_event, payload) => {
...
ipcMain.handle('zymatica:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('zymatica:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveZymaticaCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('zymatica:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveZymaticaCwd()
}))

ipcMain.handle('zymatica:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('zymatica:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('zymatica:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('zymatica:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('zymatica:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('zymatica:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: zymaticaLog.slice(-200) }))
...
function terminalShellCommand() {
  // ZYMATICA_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.ZYMATICA_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Zymatica' TUI into
  ...
  env.TERM_PROGRAM = 'Zymatica'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a zymatica/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from ZYMATICA_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.ZYMATICA_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `zymatica:terminal:${id}:${suffix}`
}
...
ipcMain.handle('zymatica:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('zymatica:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('zymatica:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('zymatica:fs:openDir', async (_event, dirPath) => {
...
ipcMain.handle('zymatica:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('zymatica:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('zymatica:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('zymatica:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('zymatica:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('zymatica:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('zymatica:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('zymatica:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('zymatica:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('zymatica:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('zymatica:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('zymatica:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('zymatica:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('zymatica:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zymatica:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zymatica:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('zymatica:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('zymatica:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('zymatica:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('zymatica:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('zymatica:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('zymatica:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('zymatica:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('zymatica:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('zymatica:terminal:write', (_event, id, data) => {
...
ipcMain.handle('zymatica:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('zymatica:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('zymatica:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('zymatica:updates:check', async () =>
...
ipcMain.handle('zymatica:updates:apply', async (_event, payload) =>
...
ipcMain.handle('zymatica:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('zymatica:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Zymatica version (the one `release.py` bumps in
// zymatica_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Zymatica version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveZymaticaVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'zymatica_cli', '__init__.py')
...
// Re-resolve the live Zymatica version and push it into the native About panel
// just before showing it, so an in-place `zymatica update` is reflected without
...
ipcMain.handle('zymatica:version', async () => ({
  appVersion: resolveZymaticaVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  zymaticaRoot: resolveUpdateRoot()
}))
...
// the actual removal via `zymatica uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (zymatica_cli/uninstall.py + zymatica_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ZYMATICA_ROOT
  ...
  const fallback = () => ({
    zymatica_home: ZYMATICA_HOME,
    agent_installed: isZymaticaSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'zymatica_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ZYMATICA_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Zymatica agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import zymatica_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_ZYMATICA_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_ZYMATICA_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_ZYMATICA_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `zymatica-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `zymatica-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('zymatica:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('zymatica:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('zymatica:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('zymatica:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// zymatica:// deep links (e.g. zymatica://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const ZYMATICA_PROTOCOL = 'zymatica'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${ZYMATICA_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // zymatica://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('zymatica:deep-link', payload)
...
ipcMain.handle('zymatica:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ZYMATICA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ZYMATICA_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(ZYMATICA_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `zymatica://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching zymatica:// URL is in our own argv.
