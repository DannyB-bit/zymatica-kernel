...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zymatica-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'zymatica-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Zymatica Test'], { cwd: dir })
