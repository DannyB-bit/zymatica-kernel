// Backend subcommand routing for the desktop-managed Zymatica process.
//
// The desktop app launches its own headless backend via `zymatica serve` — it
...
// app hasn't updated yet, or an older `zymatica` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Zymatica profile to pin via `--profile`.
 */
 ...
 * `-m zymatica_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `zymatica_cli/subcommands/dashboard.py` source registers
