// Profile-delete routing logic for the `zymatica:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_zymatica_home() and
...
/**
 * Parse a `zymatica:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `zymatica:api` request away from the primary/window backend
 ...
 * ensure_zymatica_home() recreates the directory the delete just removed.
 */
