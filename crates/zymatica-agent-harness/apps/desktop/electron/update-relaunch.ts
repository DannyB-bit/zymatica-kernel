 ...
 * Background
 * ----------
 * After `zymatica update` + `zymatica desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything ZYMATICA_DESKTOP_* is preserved
// plus ZYMATICA_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['ZYMATICA_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['ZYMATICA_DESKTOP_']
