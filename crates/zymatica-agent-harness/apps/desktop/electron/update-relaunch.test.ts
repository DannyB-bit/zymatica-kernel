 ...
 *      internals; ZYMATICA_HOME / ZYMATICA_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.zymatica/zymatica-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'zymatica')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Zymatica12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/zymatica/zymatica', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Zymatica/zymatica', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.zymatica/zymatica-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'zymatica'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'zymatica')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'zymatica://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'zymatica://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves ZYMATICA_HOME + ZYMATICA_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    ZYMATICA_HOME: '/home/u/.zymatica',
    ZYMATICA_DESKTOP_REMOTE_URL: 'http://box:9119',
    ZYMATICA_DESKTOP_REMOTE_TOKEN: 'secret',
    ZYMATICA_DESKTOP_ZYMATICA_ROOT: '/home/u/dev/zymatica',
    ZYMATICA_DESKTOP_APP_NAME: 'ZymaticaSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.zymatica/zymatica-agent/apps/desktop/release/linux-unpacked/Zymatica',
    args: ['zymatica://open/agent/42', "--note=it's fine"],
    env: { ZYMATICA_HOME: '/home/u/.zymatica', ZYMATICA_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export ZYMATICA_HOME='\/home\/u\/\.zymatica'/)
  assert.match(script, /export ZYMATICA_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Zymatica' 'zymatica:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `zymatica-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Zymatica/Zymatica',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `zymatica-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/Zymatica\/Zymatica'\n/)
})
