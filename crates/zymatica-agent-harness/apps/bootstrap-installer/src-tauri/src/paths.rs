...
//!
//! Mirrors `zymatica_constants.get_zymatica_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\zymatica
//!   macOS:   ~/.zymatica
//!   Linux:   ~/.zymatica  (override via $ZYMATICA_HOME)
//!
//! NOTE (macOS): Python's get_zymatica_home(), scripts/install.sh, and the
//! Electron desktop's resolveZymaticaHome() ALL use ~/.zymatica on macOS — there
...
/// Returns the canonical Zymatica home directory, respecting $ZYMATICA_HOME if set.
pub fn zymatica_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ZYMATICA_HOME") {
    ...
    {
        // %LOCALAPPDATA%\zymatica — matches scripts/install.ps1's $ZymaticaHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("zymatica");
        }
    }

    // macOS + Linux + fallback: ~/.zymatica (matches Python get_zymatica_home(),
    // install.sh, and the Electron desktop's resolveZymaticaHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".zymatica");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    zymatica_home().join("bootstrap-cache")
}
...
/// ZYMATICA_HOME so it survives repo checkout deletion (unlike anything under
/// zymatica-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\zymatica\zymatica-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_zymatica`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under ZYMATICA_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ZYMATICA_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    zymatica_home().join(".zymatica-update-in-progress")
}
...
pub fn copy_self_to_zymatica_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ZYMATICA_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ZYMATICA_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".zymatica-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ZYMATICA_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[zymatica-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ZYMATICA_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_zymatica_home() -> String {
    zymatica_home().to_string_lossy().into_owned()
}
