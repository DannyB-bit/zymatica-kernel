...
///
/// `zymatica_home_override` propagates to the child as $ZYMATICA_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, zymatica_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = zymatica_home_override {
        cmd.env("ZYMATICA_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, zymatica_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = zymatica_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_zymatica_home() {
