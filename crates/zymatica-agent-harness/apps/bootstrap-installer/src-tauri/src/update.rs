...
//!
//! Driven when the installer is launched as `Zymatica-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Zymatica desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `zymatica update`
//!      or repair bootstrap can race locked files),
//!   2. run `zymatica update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in zymatica_cli/main.py),
//!   3. run `zymatica desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `zymatica update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_zymatica) and
...
/// `zymatica update` exit code meaning "another zymatica process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// zymatica_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `zymatica update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let zymatica_home = crate::paths::zymatica_home();
    let install_root = zymatica_home.join("zymatica-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_zymatica`
    ...
    let zymatica = resolve_zymatica(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the zymatica CLI under {}. Is Zymatica installed? \
    ...
    // async on Windows. If it still holds the venv shim, `zymatica update`
    ...
    // ---- stage 2: zymatica update -----------------------------------------
    // Pass --branch so `zymatica update` targets the branch this installer was
    ...
    // without --branch, `zymatica update` switches the checkout to `main` (a
    ...
    // --force skips `zymatica update`'s Windows running-exe guard (which would
    ...
    // time `zymatica update` runs there is no legitimate zymatica.exe to protect,
    // and the guard would only produce a false "Zymatica is still running" stop.
    //
    ...
    // probe only checks the zymatica.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Zymatica windows" message.
    ...
    // Retry-once for the update-boundary crash. `zymatica update` lazily imports
    ...
    // second `zymatica update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Zymatica" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Zymatica is still running. Close all Zymatica windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "zymatica update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::zymatica_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: zymatica desktop --build-only ----------------------------
    // `zymatica update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `zymatica update` already does above, and `zymatica update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `zymatica desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Zymatica manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_zymatica_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Zymatica to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend zymatica.exe (or the desktop Zymatica.exe
            ...
            // force-kill every Zymatica.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Zymatica still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_zymatica();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_zymatica(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Zymatica.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Zymatica.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `zymatica.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `zymatica.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\zymatica.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startZymatica()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// zymatica.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named zymatica.exe.)
fn force_kill_other_zymatica() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `zymatica <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv zymatica shim under an install root, regardless of existence.
fn venv_zymatica(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("zymatica.exe")
    } else {
        install_root.join("venv").join("bin").join("zymatica")
    }
}

/// Resolve the zymatica CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `zymatica` on PATH.
fn resolve_zymatica(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_zymatica(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "zymatica.exe" } else { "zymatica" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let zymatica_home = crate::paths::zymatica_home();
    let mut envs = vec![(
        "ZYMATICA_HOME".to_string(),
        zymatica_home.as_os_str().to_os_string(),
    )];
    // `zymatica update` is a Python CLI writing to a pipe here, so CPython
    ...
    if let Some(path) = path_with_prepended_entries(&[
        zymatica_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_zymatica_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Zymatica.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.zymatica-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.zymatica-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::zymatica_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_zymatica_is_under_install_root() {
        let root = Path::new("/x/zymatica-agent");
        let shim = venv_zymatica(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/zymatica-agent"));
        assert!(
            envs.iter()
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_zymatica(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/zymatica-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".zymatica-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Zymatica.app"]),
            Some(PathBuf::from("/Applications/Zymatica.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Zymatica.app");
        let tmp = base.join("Zymatica.app.zymatica-update-new");
        let old = base.join("Zymatica.app.zymatica-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Zymatica.app");
        let tmp = base.join("Zymatica.app.zymatica-update-new"); // intentionally absent
        let old = base.join("Zymatica.app.zymatica-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Zymatica.app");
        let tmp = base.join("Zymatica.app.zymatica-update-new"); // absent
        let old = base.join("Zymatica.app.zymatica-update-old");
        write_marker(&target, "OLD");
