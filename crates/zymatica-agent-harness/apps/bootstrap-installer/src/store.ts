...
export const $zymaticaHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.zymatica/logs/bootstrap-installer.log')
    $zymaticaHome.set('~/.zymatica')
  ...
  try {
    const [logPath, zymaticaHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_zymatica_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $zymaticaHome.set(zymaticaHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Zymatica" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Zymatica-Setup.exe --update);
...
export async function launchZymaticaDesktop(): Promise<void> {
  ...
  await invoke('launch_zymatica_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Zymatica repository' },
