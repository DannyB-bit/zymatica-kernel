...
_ZYMATICA_CORE_TOOLS = [
    # Web
    "web_search", "web_extract",
    # Terminal + process management
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane, and close an agent's
    # read-only terminal tab (both gated on ZYMATICA_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (ZYMATICA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `zymatica tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``zymatica tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ZYMATICA_KANBAN_TASK env "
    ...
    # Coding posture (base Zymatica — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in zymatica_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Zymatica toolsets (CLI + messaging platforms)
    #
    ...
    # the `zymatica send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "zymatica-cli": {
        ...
        "tools": _ZYMATICA_CORE_TOOLS,
        "includes": []
    },

    "zymatica-cron": {
        # Mirrors zymatica-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `zymatica tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as zymatica-cli; gated by `zymatica tools`",
        "tools": _ZYMATICA_CORE_TOOLS,
        "includes": []
    },
    ...
    "zymatica-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _ZYMATICA_CORE_TOOLS,
        "includes": []
    },

    "zymatica-homeassistant": {
    ...
    "zymatica-email": {
        "description": "Email bot toolset - interact with Zymatica via email (IMAP/SMTP)",
        "tools": _ZYMATICA_CORE_TOOLS,
        "includes": []
    },
    ...
    "zymatica-sms": {
        "description": "SMS bot toolset - interact with Zymatica via SMS (Twilio)",
        "tools": _ZYMATICA_CORE_TOOLS,
        "includes": []
    },
    ...
    "zymatica-webhook": {
        ...
        "tools": _ZYMATICA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "zymatica-gateway": {
        ...
        "tools": [],
        "includes": ["zymatica-telegram", "zymatica-discord", "zymatica-whatsapp", "zymatica-slack", "zymatica-signal", "zymatica-bluebubbles", "zymatica-homeassistant", "zymatica-email", "zymatica-sms", "zymatica-mattermost", "zymatica-matrix", "zymatica-dingtalk", "zymatica-feishu", "zymatica-wecom", "zymatica-wecom-callback", "zymatica-weixin", "zymatica-qqbot", "zymatica-webhook", "zymatica-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``zymatica-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ZYMATICA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``zymatica-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (zymatica-<name>).
        # Gives them _ZYMATICA_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("zymatica-"):
            platform_name = name[len("zymatica-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ZYMATICA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
