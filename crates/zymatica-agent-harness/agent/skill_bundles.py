...
Storage
-------
Bundles live in ``~/.zymatica/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ZYMATICA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``zymatica bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ZYMATICA_HOME.

    Honors ``ZYMATICA_BUNDLES_DIR`` for tests; falls back to
    ``<ZYMATICA_HOME>/skill-bundles``.
    """
    override = os.environ.get("ZYMATICA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_zymatica_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `zymatica bundles` CLI subcommand.
# ---------------------------------------------------------------------------
