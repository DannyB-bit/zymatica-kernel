...
row taxonomy is inferred from the atlas shape so Zymatica can render both legacy
...
# rows (1536x1872), while older Zymatica/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Zymatica' activity state names. They are not always identical to the
...
# Legacy Zymatica/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Zymatica activity names -> accepted row-name aliases in descending
