...
# (Zymatica Portal → OpenAI → OpenRouter → …). OpenRouter/Nous run a quality-first
...
def _forced_provider_from_env() -> str | None:
    ...
    `ZYMATICA_PET_IMAGE_PROVIDER=<name>` (e.g. `openrouter`) bypasses the normal
    active/default provider resolution for pet generation only. Unknown values are
    ignored so existing users are unaffected.
    """
    forced = os.environ.get("ZYMATICA_PET_IMAGE_PROVIDER", "").strip().lower()
...
def resolve_provider(*, require_references: bool = True, prefer: str | None = None) -> SpriteProvider:
    ...
    raise GenerationError(
        ...
        "Open `zymatica tools` → Image Generation and configure Zymatica Portal, "
