"""Upload a Zymatica session transcript to Hugging Face as an agent trace.

Zymatica stores sessions in its own SQLite store (``zymatica_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``zymatica trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Zymatica' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "zymatica-traces"
_ZYMATICA_VERSION = "zymatica-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Zymatica OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Zymatica' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Zymatica conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.zymatica/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `zymatica trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/zymatica-traces`` dataset. Returns a
