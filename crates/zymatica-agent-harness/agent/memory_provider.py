...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `zymatica backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - zymatica_home (str): The active ZYMATICA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.zymatica``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "zymatica").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'zymatica memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], zymatica_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'zymatica memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``zymatica_home`` is the active ZYMATICA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ZYMATICA_HOME.

        ``zymatica backup`` only walks ZYMATICA_HOME, so any provider state kept
        ...
        of the archive, and ``zymatica import`` restores them to their original
