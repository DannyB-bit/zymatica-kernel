...
from zymatica_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_nous_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    """
    try:
        from zymatica_cli.zymatica_account import zymatica_portal_topup_url
        ...
        details.append(f"Top up: {zymatica_portal_topup_url(account_info)}")
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Account-independent of any live agent: gated on "a Zymatica account is logged in"
    ...
    Dev override: when ZYMATICA_DEV_CREDITS_FIXTURE selects a fixture state, /usage
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                get_zymatica_portal_account_info, force_fresh=True
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so ZYMATICA_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — ZYMATICA_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="nous",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    try:
        ...
        from zymatica_cli.zymatica_account import (
            ...
            zymatica_portal_topup_url,
        )

        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_zymatica_portal_account_info, force_fresh=True).result(
                timeout=timeout
            )
    except Exception:
    ...
    return CreditsView(
        ...
        topup_url=zymatica_portal_topup_url(account),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Zymatica's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `zymatica auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `zymatica auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
