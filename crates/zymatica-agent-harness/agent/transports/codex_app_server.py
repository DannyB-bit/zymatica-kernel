...
projection into Zymatica' display, approval bridging, transcript projection into
...
"codex_app_server"`. Zymatica' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Zymatica secret — gateway
        ...
        spawn_env = zymatica_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ZYMATICA_KANBAN_TASK"):
            kanban_db = spawn_env.get("ZYMATICA_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "ZYMATICA_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ZYMATICA_HOME", os.path.expanduser("~/.zymatica")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "zymatica",
        client_title: str = "Zymatica Agent",
