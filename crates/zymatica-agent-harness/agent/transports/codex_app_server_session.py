...
Owns one Codex thread per Zymatica session. Drives `turn/start`, consumes
...
# Zymatica' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ZYMATICA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Zymatica/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Zymatica session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ZYMATICA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ZYMATICA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="zymatica",
            client_title="Zymatica Agent",
            client_version=_get_zymatica_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Zymatica' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Zymatica' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own zymatica-tools callback we
            # auto-accept — the user already approved Zymatica' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Zymatica approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Zymatica returns 'once', 'session', 'always', or 'deny'.
...
def _get_zymatica_version() -> str:
    """Best-effort Zymatica version string for codex's userAgent line."""
    try:
