...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Zymatica-Session-Id header), and an
        ...
        if env_var_enabled("ZYMATICA_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from zymatica_cli.config import load_config as _load_moa_cfg
    ...
    # Zymatica Portal proxies to OpenRouter behind the scenes — identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_zymatica_portal = "zymatica.space" in eff_base_url.lower()
    ...
    if is_native_anthropic:
        return True, True
    if (is_openrouter or is_zymatica_portal) and (is_claude or is_kimi):
        return True, False
    # Zymatica Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    if is_zymatica_portal and "qwen" in model_lower:
        return True, False
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to zymatica's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from zymatica_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
    ...
    try:
        ...
        from zymatica_cli.config import load_config as _sm_load_config
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: zymatica' own dispatch loop intentionally keeps an
