"""Anthropic Messages API adapter for Zymatica Agent.

Translates between Zymatica's internal OpenAI-style message format and
...
# Zymatica effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See zymatica-agent#13848, #17057.
    """
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to zymatica's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to zymatica's outer conversation
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Zymatica notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Zymatica historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``zymatica models``) never mutates ``~/.zymatica/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Zymatica)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.zymatica/auth.json)
    ...
    # 1. Zymatica-managed OAuth/setup token env var
    ...
    # 4. Zymatica credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Zymatica configs.
...
# ── Zymatica-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.zymatica/.anthropic_oauth.json (our own file).
...
def _get_zymatica_oauth_file() -> Path:
    return get_zymatica_home() / ".anthropic_oauth.json"
...
def run_zymatica_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Zymatica-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Zymatica with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from zymatica_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_zymatica_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Zymatica-managed OAuth credentials from ~/.zymatica/.anthropic_oauth.json."""
    oauth_file = _get_zymatica_oauth_file()
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Zymatica OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See zymatica-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See zymatica-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See zymatica-agent#13848 (Kimi) and
    zymatica-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Zymatica Agent", "Claude Code")
                text = text.replace("Zymatica agent", "Claude Code")
                text = text.replace("zymatica-agent", "claude-code")
                text = text.replace("Zymatica Team", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Zymatica-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Zymatica surfaces in its CLI. We
