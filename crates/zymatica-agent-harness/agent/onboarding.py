...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Zymatica starts and finds ``~/.openclaw/``.

    Points users at ``zymatica claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``zymatica claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Zymatica, run "
        "`zymatica claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `zymatica claw cleanup` (renames it to ~/.openclaw.pre-migration — "
