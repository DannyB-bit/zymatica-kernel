...
# un-throttled log fires on *every* selection. On Windows several Zymatica
...
# readiness handshake ("Timed out connecting to Zymatica backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``zymatica_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``zymatica model`` / ``zymatica auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Zymatica process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        avoiding a "refresh token reuse" revocation on the Zymatica Portal.
        """
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``zymatica`` invocation that defaults to the active provider
        (e.g. setup wizard, ``zymatica auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # zymatica_cli.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Zymatica
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``ZYMATICA_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "ZYMATICA_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "ZYMATICA_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("zymatica_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Zymatica
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("zymatica_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Zymatica process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Zymatica profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `zymatica model` / `zymatica auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `zymatica model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``zymatica auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `zymatica auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `zymatica auth remove <provider> <N>` is stable across all source types.
    try:
        from zymatica_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Zymatica PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `zymatica setup` ("Claude
        ...
        # Claude Code CLI, zymatica_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.zymatica/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `zymatica setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"zymatica_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_zymatica_oauth_credentials

        for source_name, creds in (
            ("zymatica_pkce", read_zymatica_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `zymatica auth add nous --label <name>`).  Fall back to the
        ...
        # Zymatica auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from zymatica_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.zymatica/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `zymatica auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `zymatica auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Zymatica auth store.  Without this gate the removal is instantly
        ...
        # Zymatica owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `zymatica auth openai-codex`.
        ...
        # When the user logs in via ``zymatica model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``zymatica auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from zymatica_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.zymatica/.env over os.environ — the user's config file is the
    # authoritative source for Zymatica credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_zymatica_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `zymatica auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.zymatica/.env.
    ...
    try:
        from zymatica_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    if provider == "openrouter":
        # Prefer ~/.zymatica/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `zymatica auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Zymatica
