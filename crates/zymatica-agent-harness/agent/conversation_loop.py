...
from zymatica_constants import PARTIAL_STREAM_STUB_ID
from zymatica_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Zymatica tool use: "
    ...
    return (
        ...
        f"context, but Zymatica needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Zymatica config, set `model.ollama_num_ctx: 65536` "
...
def _nous_entitlement_message(capability: str) -> str:
    try:
        from zymatica_cli.zymatica_account import (
            format_zymatica_portal_entitlement_message,
            get_zymatica_portal_account_info,
        )

        account_info = get_zymatica_portal_account_info(force_fresh=True)
        message = format_zymatica_portal_entitlement_message(
            account_info,
...
def _is_nous_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "api.zymatica.space")
    )
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `zymatica fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from zymatica_cli.moa_config import decode_moa_turn
    ...
    # all run inside Codex). Default Zymatica path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Zymatica internals.
        #
        # Zymatica invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Zymatica tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── Zymatica Portal rate limit guard ──────────────────────
            ...
            if agent.provider == "nous":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        _nous_msg = (
                            f"Zymatica Portal rate limit active — "
            ...
            try:
                ...
                try:
                    from zymatica_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("ZYMATICA_DUMP_REQUESTS"):
                ...
                from zymatica_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Zymatica/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    from zymatica_constants import display_zymatica_home as _dhh_fn
                    _dhh = _dhh_fn()
                    _body_text = ""
                    try:
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: zymatica auth add nous")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.zymatica.space")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `zymatica doctor` for credential-chain diagnostics, or")
                    ...
                    from zymatica_constants import display_zymatica_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Zymatica-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: zymatica config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: zymatica config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── Zymatica Portal: record rate limit & skip retries ─────
                ...
                #
                # IMPORTANT: Zymatica Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Zymatica).  A 429 can
                ...
                # which Zymatica' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Zymatica' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`zymatica",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Zymatica may add API-only content not present in persisted
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `zymatica auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `zymatica model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Zymatica Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: zymatica portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.zymatica.space", force=True)
                                # ``:free`` is OpenRouter slug syntax; Zymatica Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Zymatica Portal won't recognize that model name. Either switch to a", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: zymatica setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        zymatica fallback add   (interactive picker — same as `zymatica model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a Zymatica/gateway failure).\n\n"
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.zymatica/config.yaml` to extend the per-call "
                            f"timeout. (Zymatica's built-in floor is 600s for "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Zymatica mid-turn, resume logic still sees the
                ...
                try:
                    ...
                    from zymatica_cli.plugins import get_pre_verify_continue_message, has_hook
