"""Cross-session rate limit guard for Zymatica Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Zymatica Portal is currently rate-limited
...
(3 SDK retries x 3 Zymatica retries), and every one of those calls counts
...
def _state_path() -> str:
    ...
    try:
        ...
        base = get_zymatica_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".zymatica")
...
def record_nous_rate_limit(
    *,
    ...
    """Record that Zymatica Portal is rate-limited.
...
def nous_rate_limit_remaining() -> Optional[float]:
    """Check if Zymatica Portal is currently rate-limited.
...
def is_genuine_nous_rate_limit(
    *,
    ...
    """Decide whether a 429 from Zymatica Portal is a real account rate limit.

    Zymatica Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Zymatica, ...) behind one endpoint.  A 429 can mean either:
