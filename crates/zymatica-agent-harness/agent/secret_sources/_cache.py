...
cache problem must never block Zymatica startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Zymatica home used for cache paths.

    ``home_path`` is whatever ``load_zymatica_dotenv()`` already resolved;
    falling back to ``$ZYMATICA_HOME`` / ``~/.zymatica`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("ZYMATICA_HOME", Path.home() / ".zymatica"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<zymatica_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ZYMATICA_HOME is ever made traversable.
            try:
