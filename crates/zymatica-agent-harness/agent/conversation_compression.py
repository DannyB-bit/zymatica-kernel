...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``zymatica_state.SessionDB`` class (and its live instances) is old.
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `zymatica setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Zymatica "
...
def compress_context(
    ...
    *,
    ...
    # Zymatica' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|zymatica|off).
    ...
    try:
        ...
        if agent._session_db:
            try:
                ...
                if in_place:
                    ...
                    # for search/recovery (Zymatica review — keep one durable id
                    ...
                    # from ``zymatica_logging._session_context`` (set once per turn in
                    ...
                    try:
                        agent._session_db.create_session(
                            ...
                            source=agent.platform or os.environ.get("ZYMATICA_SESSION_SOURCE", "cli"),
                            model=agent.model,
                    ...
                    try:
                        from zymatica_cli.goals import migrate_goal_to_session
        ...
        # engines (e.g. zymatica-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See zymatica-lcm#68. Built-in ContextCompressor
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Zymatica' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Zymatica' transcript
    unchanged.
    """
    auto_mode = str(
        getattr(agent, "codex_app_server_auto_compaction", "native") or "native"
    ).lower()
    if auto_mode not in {"native", "zymatica", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "zymatica":
        logger.info(
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="zymatica_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
