...
from zymatica_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ZYMATICA_KANBAN_TASK`` / ``ZYMATICA_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ZYMATICA_KANBAN_TASK") or os.getenv("ZYMATICA_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Zymatica Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``zymatica_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ZYMATICA_PLATFORM`` or
            ``ZYMATICA_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("ZYMATICA_PLATFORM")
        or get_session_env("ZYMATICA_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``zymatica`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.zymatica/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``zymatica`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ZYMATICA_HOME, not cwd
        if not p.is_absolute():
            p = (zymatica_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.zymatica/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.zymatica/skills/`` (including via symlinks) or configured
    ...
    # ~/.zymatica/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Zymatica can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    zymatica = metadata.get("zymatica") or {}
    if not isinstance(zymatica, dict):
        zymatica = {}
    return {
        "fallback_for_toolsets": zymatica.get("fallback_for_toolsets", []),
        "requires_toolsets": zymatica.get("requires_toolsets", []),
        "fallback_for_tools": zymatica.get("fallback_for_tools", []),
        "requires_tools": zymatica.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    zymatica = metadata.get("zymatica")
    if not isinstance(zymatica, dict):
        return []
    raw = zymatica.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Zymatica metadata, VCS, virtualenv/dependency, cache, and skill
