...
from zymatica_constants import get_zymatica_home, get_skills_dir, is_wsl
...
_ZYMATICA_MD_NAMES = (".zymatica.md", "ZYMATICA.md")


def _find_zymatica_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.zymatica.md`` or ``ZYMATICA.md``.
    ...
    # could pick up a .zymatica.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _ZYMATICA_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Zymatica Agent, an intelligent AI assistant created by Zymatica Team. "
...
ZYMATICA_AGENT_HELP_GUIDANCE = (
    "You run on Zymatica Agent (by Zymatica Team). When the user needs help with "
    "Zymatica itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://zymatica.space/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `zymatica-agent` skill with skill_view(name='zymatica-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.zymatica/kanban.db`. Your task id is in "
    "`$ZYMATICA_KANBAN_TASK`; your workspace is `$ZYMATICA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ZYMATICA_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ZYMATICA_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ZYMATICA_KANBAN_BRANCH:-wt/$ZYMATICA_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ZYMATICA_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`zymatica profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `zymatica kanban <verb>` for board operations. Use "
...
#
# The zymatica-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to zymatica-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `zymatica computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Zymatica "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Zymatica terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Zymatica desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Zymatica WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Zymatica itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Zymatica runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Zymatica itself is running. The host OS, home, and cwd "
                f"of the Zymatica process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Zymatica "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Zymatica
    ...
    extra = (os.getenv("ZYMATICA_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_zymatica_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ZYMATICA_PLATFORM") or os.environ.get("ZYMATICA_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.zymatica/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Zymatica Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `zymatica-agent` skill "
            "first. It has the actual commands (e.g. `zymatica config set …`, `zymatica tools`, "
            "`zymatica setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact Zymatica subscription capability block for the system prompt."""
    try:
        from zymatica_cli.nous_subscription import get_nous_subscription_features
        from tools.tool_backend_helpers import managed_nous_tools_enabled
    except Exception as exc:
        logger.debug("Failed to import Zymatica subscription helper: %s", exc)
        return ""
    ...
    def _status_line(feature) -> str:
        if feature.managed_by_nous:
            return f"- {feature.label}: active via Zymatica subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.nous_auth_present:
            return f"- {feature.label}: included with Zymatica subscription, not currently selected"
        if feature.key == "modal" and features.nous_auth_present:
            return f"- {feature.label}: optional via Zymatica subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Nous Subscription",
        "Zymatica subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, OpenAI Whisper STT, and browser automation (Browser Use) by default. Modal execution is optional.",
    ...
    lines.extend(
        [
            ...
            "If the user is not subscribed and asks for a capability that Zymatica subscription would unlock or simplify, suggest Zymatica subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: zymatica setup, zymatica setup tools, zymatica setup terminal, zymatica status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ZYMATICA_HOME and return its content, or None.
    ...
    """
    try:
        from zymatica_cli.config import ensure_zymatica_home
        ensure_zymatica_home()
    except Exception as e:
        logger.debug("Could not ensure ZYMATICA_HOME before loading SOUL.md: %s", e)

    soul_path = get_zymatica_home() / "SOUL.md"
...
def _load_zymatica_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.zymatica.md / ZYMATICA.md — walk to git root."""
    zymatica_md_path = _find_zymatica_md(cwd_path)
    if not zymatica_md_path:
        return ""
    try:
        content = zymatica_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = zymatica_md_path.name
        try:
            rel = str(zymatica_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".zymatica.md", context_length=context_length,
            read_path=str(zymatica_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", zymatica_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .zymatica.md / ZYMATICA.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ZYMATICA_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Zymatica install/source
    ...
    # explicitly configured cwd is honored verbatim — the Zymatica tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Zymatica in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Zymatica install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_zymatica_md(cwd_path, context_length)
    ...
    # SOUL.md from ZYMATICA_HOME only — skip when already loaded as identity
    if not skip_soul:
