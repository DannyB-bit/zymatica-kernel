...
Plugins built on Zymatica Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Zymatica
...
:class:`~zymatica_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Zymatica' main config
...
handles every provider, fallback chain, and per-task override Zymatica
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`zymatica_cli.plugins.PluginContext`
