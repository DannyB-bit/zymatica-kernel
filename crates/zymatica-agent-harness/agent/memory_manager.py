...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_ZYMATICA_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Zymatica expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``zymatica_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_zymatica_home()`` themselves.
        """
        if "zymatica_home" not in kwargs:
            from zymatica_constants import get_zymatica_home
            kwargs["zymatica_home"] = str(get_zymatica_home())
        for provider in self._providers:
            try:
