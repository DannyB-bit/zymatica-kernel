...
from zymatica_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from zymatica_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("ZYMATICA_STREAM_STALE_GIVEUP", 5)
...
def _derive_stream_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    if _cfg_stale is not None:
        ...
        _base = env_float("ZYMATICA_STREAM_STALE_TIMEOUT", 180.0)
...
def direct_api_call(agent, api_kwargs: dict):
    ...
    timeout (provider ``request_timeout_seconds`` / ``ZYMATICA_API_TIMEOUT``) bounds
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # activity. Operators can tune via ZYMATICA_CODEX_TTFB_TIMEOUT_SECONDS and
    # ZYMATICA_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # limit. Tunable via ZYMATICA_CODEX_HARD_TIMEOUT_SECONDS (set to 0 to
    # disable the ceiling entirely; that restores the pre-fix behavior).
    _codex_hard_timeout = _env_float("ZYMATICA_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
    if (
    ...
    # ZYMATICA_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("ZYMATICA_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("ZYMATICA_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        _ttfb_strict = os.environ.get("ZYMATICA_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            if _ttfb_timeout < _large_request_ttfb_timeout:
                logger.info(
                    ...
                    "Set ZYMATICA_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.",
                    _ttfb_timeout,
        ...
        _ttfb_cap = _env_float("ZYMATICA_CODEX_TTFB_MAX_SECONDS", 120.0)
        if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
            logger.info(
                "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens). Set ZYMATICA_CODEX_TTFB_MAX_SECONDS to tune.",
                _ttfb_timeout,
                _ttfb_cap,
                f"{_est_tokens_for_codex_watchdog:,}",
            )
    ...
    _codex_idle_timeout = _env_float(
        "ZYMATICA_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    _is_nous = "zymatica.space" in agent._base_url_lower
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "zymatica",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects ZYMATICA_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        if not fb_api_key_hint:
            ...
            # _normalize_custom_provider_entry in zymatica_cli/config.py).
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every Zymatica-internal underscore-prefixed scaffolding key.
            ...
            # substitute it before dropping the key (Zymatica bookkeeping,
        ...
        if _is_nous:
            from agent.portal_tags import zymatica_portal_tags as _portal_tags
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    if agent.api_mode == "bedrock_converse":
        ...
        while t.is_alive():
            ...
            if _stale_elapsed > _bedrock_stale_timeout:
                ...
                # crosses ZYMATICA_STREAM_STALE_GIVEUP, so a persistently wedged
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # wins over the ZYMATICA_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else env_float("ZYMATICA_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # ZYMATICA_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("ZYMATICA_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode ZYMATICA_STREAM_READ_TIMEOUT.
    ...
    def _call():
        ...
        _max_stream_retries = env_int("ZYMATICA_STREAM_RETRIES", 2)
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("ZYMATICA_STREAM_STALE_TIMEOUT", 180.0)
    ...
    # unless the user explicitly set ZYMATICA_STREAM_STALE_TIMEOUT; override the
    # local ceiling with ZYMATICA_LOCAL_STREAM_STALE_TIMEOUT (documented in
    # website/docs/reference/environment-variables.md).
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        # Read config.yaml ``agent.local_stream_stale_timeout`` (default 900),
        # env var ``ZYMATICA_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch.
        ...
        try:
            ...
            pass
        _stream_stale_timeout = env_float("ZYMATICA_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
        logger.debug(
