...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    ...
    Dev override: ``ZYMATICA_DEV_CREDITS_FIXTURE`` short-circuits to a fixture so
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_zymatica_portal_account_info, force_fresh=True).result(timeout=timeout)
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """Map ``ZYMATICA_DEV_CREDITS_FIXTURE`` to a usage model for offline UX work.

    Recognized names: ``free | healthy | low | topup | depleted``. Returns
    ``None`` when the env var is unset (real portal path runs).
    """
    name = (os.getenv("ZYMATICA_DEV_CREDITS_FIXTURE") or "").strip().lower()
    if not name:
        return None
