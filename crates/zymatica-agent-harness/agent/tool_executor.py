...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("ZYMATICA_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid ZYMATICA_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Zymatica process before the normal turn-end persistence path runs.
