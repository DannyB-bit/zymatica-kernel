...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Zymatica with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.zymatica/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Zymatica
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zymatica tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``zymatica setup`` for
