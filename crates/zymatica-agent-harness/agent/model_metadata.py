...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Zymatica also honours ZYMATICA_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ZYMATICA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_zymatica_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Zymatica Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Zymatica from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Zymatica' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Zymatica minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from zymatica_constants import get_zymatica_home
    return get_zymatica_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Zymatica grow conversations past the runtime limit and Ollama
...
def _resolve_nous_context_length(
    ...
    """Resolve Zymatica Portal model context length.
...
def get_model_context_length(
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # leftover — drop it and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Zymatica Portal: the portal /v1/models endpoint is authoritative.
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from zymatica_cli.models import get_copilot_model_context
    ...
    # ``Zymatica-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Zymatica sends to providers:
