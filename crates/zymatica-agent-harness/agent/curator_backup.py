...
A pre-run snapshot of ``~/.zymatica/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.zymatica/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.zymatica/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_zymatica_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.zymatica/cron/jobs.json``)."""
    return get_zymatica_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.zymatica/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.zymatica/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.zymatica/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.zymatica/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `zymatica curator rollback --list` to see available snapshots)",
            None,
        )
