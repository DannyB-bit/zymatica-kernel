"""Centralized Zymatica Portal request tags.

Every Zymatica request that hits the Zymatica Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Zymatica Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=zymatica-agent",
        "client=zymatica-client-v<__version__>",
    ]

The version is sourced live from ``zymatica_cli.__version__`` so it auto-aligns
...
``zymatica_cli.__version__`` is the canonical source of truth.
"""
...
# active conversation id here and ``zymatica_portal_tags()`` picks it up as a
...
_conversation_id: ContextVar[Optional[str]] = ContextVar(
    "zymatica_portal_conversation_id", default=None
)
...
def _zymatica_version() -> str:
    """Return the current Zymatica release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``zymatica_cli`` cannot be imported (should
...
def zymatica_client_tag() -> str:
    """Return the ``client=...`` tag for Zymatica Portal requests.

    Format: ``client=zymatica-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=zymatica-client-v{_zymatica_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Zymatica session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Zymatica conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.zymatica/sessions/`` storage, session logs, and lineage.
...
def zymatica_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of Zymatica Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(zymatica_portal_tags())``).

    When ``session_id`` is provided, a ``conversation=<session_id>`` tag is
    appended so Portal usage can be attributed to a specific Zymatica
    ...
    """
    tags = ["product=zymatica-agent", zymatica_client_tag()]
