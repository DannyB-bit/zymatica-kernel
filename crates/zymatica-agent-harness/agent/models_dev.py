...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.zymatica/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Zymatica ↔ models.dev
# ---------------------------------------------------------------------------

# Zymatica provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Zymatica (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_zymatica_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``zymatica config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Zymatica provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Zymatica agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Zymatica provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Zymatica ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Zymatica or models.dev provider ID.  Tries exact match then
