...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ZYMATICA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Zymatica runtime / deployment
    "ZYMATICA_HOME", "ZYMATICA_PROFILE", "ZYMATICA_GATEWAY_LOCK_DIR",
    "ZYMATICA_MAX_ITERATIONS", "ZYMATICA_MAX_TOKENS", "ZYMATICA_API_TIMEOUT",
    "ZYMATICA_REDACT_SECRETS", "ZYMATICA_NOUS_TIMEOUT_SECONDS",
    "_ZYMATICA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ZYMATICA_KANBAN_DB", "ZYMATICA_KANBAN_WORKSPACES_ROOT", "ZYMATICA_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "ZYMATICA_KANBAN_",
    "ZYMATICA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(zymatica_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(zymatica_home) / ".env")
