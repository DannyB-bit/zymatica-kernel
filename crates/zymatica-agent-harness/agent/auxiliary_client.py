...
Resolution order for text tasks (auto mode):
  ...
  3. Zymatica Portal (~/.zymatica/auth.json active provider)
...
from zymatica_cli.config import get_zymatica_home
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``ZYMATICA_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `zymatica update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Zymatica owns auxiliary retry + provider/model fallback policy (the
    ...
    # Zymatica sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Zymatica control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Zymatica triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://zymatica.space",
    "X-Title": "Zymatica Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ZYMATICA_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ZYMATICA_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ZYMATICA_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ZYMATICA_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "ZymaticaAgent",
}
...
# Zymatica Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Zymatica Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with zymatica_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from agent.portal_tags import zymatica_portal_tags as _zymatica_portal_tags


def _nous_extra_body() -> dict:
    """Return a fresh Zymatica Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``zymatica_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _zymatica_portal_tags()}
...
# ``_nous_extra_body()`` or import ``zymatica_portal_tags`` directly.
NOUS_EXTRA_BODY = _nous_extra_body()

# Set at resolve time — True if the auxiliary client points to Zymatica Portal
auxiliary_is_nous: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_NOUS_MODEL = "google/gemini-3-flash-preview"
_NOUS_DEFAULT_BASE_URL = "https://api.zymatica.space/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_zymatica_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Zymatica Agent)",
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("ZYMATICA_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Zymatica plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``zymatica_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.zymatica/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from zymatica_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from zymatica_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        ...
        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("ZYMATICA_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``zymatica auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``zymatica_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from zymatica_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Zymatica auth store.
    ...
    try:
        from zymatica_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from zymatica_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from zymatica_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Zymatica Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    logger.debug("Auxiliary client: Zymatica Portal")
    ...
    model = _NOUS_MODEL
    try:
        from zymatica_cli.models import get_nous_recommended_aux_model
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the Zymatica Portal's recommended model after a stale-model 404.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `zymatica setup` or `zymatica model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`zymatica_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from zymatica_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second zymatica
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``zymatica config aux reset``)."""
...
def _zymatica_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Zymatica account API says paid access is allowed."""
    try:
        from zymatica_cli.zymatica_account import get_zymatica_portal_account_info

        account_info = get_zymatica_portal_account_info(force_fresh=True)
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "api.zymatica.space"):
        return "nous"
...
def _auth_refresh_provider_for_route(
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "api.zymatica.space"):
        return "nous"
    return normalized
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Zymatica' built-in discovery
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `zymatica model` but the
    #    old OPENAI_BASE_URL lingers in ~/.zymatica/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: zymatica model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.zymatica/.env",
    ...
    if main_provider == "moa":
        try:
            from zymatica_cli.config import load_config
            from zymatica_cli.moa_config import resolve_moa_preset
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from zymatica_cli.runtime_provider import _get_named_custom_provider
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Zymatica owns
...
def resolve_provider_client(
    ...
    # ── Zymatica Native Engine (Local Gemma) ─────────────────────
    if provider == "zymatica":
        final_model = model or "zymatica/gemma-4-e4b-compressed"
        base_url = explicit_base_url or "http://127.0.0.1:9119/v1"
        try:
            from openai import OpenAI, AsyncOpenAI
            if async_mode:
                client = AsyncOpenAI(api_key="zymatica-local", base_url=base_url)
            else:
                client = OpenAI(api_key="zymatica-local", base_url=base_url)
            return _wrap_if_needed(client, final_model, base_url_str=base_url), final_model
        except Exception as e:
            logger.warning("resolve_provider_client: failed to init zymatica local client: %s", e)
            return None, None
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Zymatica Portal not configured (run: zymatica auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: zymatica model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: zymatica model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: zymatica doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("zymatica_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from zymatica_cli.models import _should_use_copilot_responses_api
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #   3. Zymatica Portal (vision-capable aggregator fallback)
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Zymatica Portal product tags when the auxiliary client is backed
    by Zymatica Portal. Returns empty dict otherwise.
    """
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`zymatica_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    if provider == "nous" and "tags" not in merged_extra:
        merged_extra["tags"] = _zymatica_portal_tags()
    if merged_extra:
        kwargs["extra_body"] = merged_extra

    # Native Anthropic Messages adapters do not consume ``extra_body``. Carry
    # the normalized Zymatica reasoning config through a private kwarg so the
...
def call_llm(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Zymatica reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `zymatica model`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (Zymatica Portal recommendation drift) ───
        ...
        _heal_is_nous = (
            ...
            or base_url_host_matches(_base_info, "api.zymatica.space")
        )
        ...
        if (
            ...
            and client_is_nous
            and _zymatica_portal_account_has_fresh_paid_access()
        ):
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        _heal_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "api.zymatica.space")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_nous_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
