...
upstream prefix cache warm.  See ``zymatica-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ZYMATICA_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ZYMATICA_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Zymatica never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ZYMATICA_HOME persona while keeping
    ...
    # Pointer to the zymatica-agent skill + docs for user questions about Zymatica itself.
    stable_parts.append(ZYMATICA_AGENT_HELP_GUIDANCE)
    ...
    # ZYMATICA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Zymatica, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Zymatica profile the agent is running
    # under so it doesn't conflate ~/.zymatica/skills/ (default profile) with
    # ~/.zymatica/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Zymatica profile: default. Other profiles (if any) live "
            "under " + str(get_zymatica_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Zymatica profile: {active_profile}. This session reads "
            f"and writes {get_zymatica_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_zymatica_home()}/skills/, {get_zymatica_home()}/plugins/, "
            f"{get_zymatica_home()}/cron/, {get_zymatica_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Zymatica). Every other surface (desktop chat panel,
    ...
    from zymatica_time import now as _zymatica_now
    now = _zymatica_now()
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Zymatica never rebuilds or reinjects parts of it
