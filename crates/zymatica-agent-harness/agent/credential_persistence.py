...
``auth.json``.  They intentionally have no dependency on ``zymatica_cli.auth`` so
...
# Sources Zymatica owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Zymatica-owned OAuth/device-code state)
