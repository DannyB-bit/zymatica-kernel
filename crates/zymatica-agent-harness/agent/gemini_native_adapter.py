...
Zymatica keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Zymatica's multi-turn
...
try:
    import zymatica_cli as _zymatica_cli

    _ZYMATICA_VERSION = str(_zymatica_cli.__version__)
except Exception:
    _ZYMATICA_VERSION = "0.0.0"
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Zymatica)
        ...
        with httpx.Client(timeout=timeout) as client:
            resp = client.post(
                ...
                headers={
                    ...
                    "X-Goog-Api-Client": f"zymatica-agent/{_ZYMATICA_VERSION}",
                },
            )
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Zymatica typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Zymatica retry/error classification."""
...
def build_gemini_request(
    *,
    ...
    if max_tokens is not None:
        ...
        # finishReason=MAX_TOKENS, truncating tool calls mid-stream (Zymatica
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.zymatica/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `zymatica setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            # Include Zymatica client context following Gemini's partner
            # integration guidance.
            # See https://ai.google.dev/gemini-api/docs/partner-integration
            "User-Agent": f"zymatica-agent/{_ZYMATICA_VERSION} (gemini-native)",
            "X-Goog-Api-Client": f"zymatica-agent/{_ZYMATICA_VERSION}",
        }
