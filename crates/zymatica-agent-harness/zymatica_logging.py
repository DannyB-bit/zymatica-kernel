"""Centralized logging setup for Zymatica Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.zymatica/logs/`` (profile-aware via ``get_zymatica_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``zymatica_cli.web_server``,
    ``zymatica_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Zymatica (TUI, gateway, ``hy_memory`` server, MCP
...
from zymatica_constants import get_config_path, get_zymatica_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_zymatica_session_injector", False):
    ...
    _session_record_factory._zymatica_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``zymatica logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "zymatica_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("zymatica_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "zymatica_cli.web_server",
        "zymatica_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    zymatica_home: Optional[Path] = None,
    ...
    """Configure the Zymatica logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    zymatica_home
        Override for the Zymatica home directory.  Falls back to
        ``get_zymatica_home()`` (profile-aware).
    log_level
    ...
    home = zymatica_home or get_zymatica_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_zymatica_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._zymatica_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the zymatica group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from zymatica_cli.config import is_managed
