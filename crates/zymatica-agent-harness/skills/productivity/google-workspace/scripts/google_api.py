#!/usr/bin/env python3
"""Google Workspace API CLI for Zymatica Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Zymatica-facing JSON contract and falls back to the Python client
...
from _zymatica_home import get_zymatica_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZYMATICA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Zymatica Agent")
