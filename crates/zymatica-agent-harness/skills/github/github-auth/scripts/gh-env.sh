#!/usr/bin/env bash
# GitHub environment detection helper for Zymatica Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _zymatica_env="${ZYMATICA_HOME:-$HOME/.zymatica}/.env"; [ -f "$_zymatica_env" ] && grep -q "^GITHUB_TOKEN=" "$_zymatica_env" 2>/dev/null; then
