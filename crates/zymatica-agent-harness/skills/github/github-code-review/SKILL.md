...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _zymatica_env="${ZYMATICA_HOME:-$HOME/.zymatica}/.env"; [ -f "$_zymatica_env" ] && grep -q "^GITHUB_TOKEN=" "$_zymatica_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_zymatica_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Zymatica Agent\",
    \"comments\": [
...
```bash
source "${ZYMATICA_HOME:-$HOME/.zymatica}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Zymatica Agent. Code looks clean — good test coverage, no security concerns."
