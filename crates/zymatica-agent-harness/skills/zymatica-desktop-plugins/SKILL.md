...
# Zymatica Desktop Plugins Skill

Write plugins for the Zymatica desktop app: statusbar items, layout panes,
...
system (`~/.zymatica/plugins/`) is otherwise documented separately.
...
- The Zymatica desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ZYMATICA_HOME/desktop-plugins/` (usually
  `~/.zymatica/desktop-plugins/`).

## How to Run

1. Create `$ZYMATICA_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (relative to this skill directory) — that's
   `~/.zymatica/...` by default, or `~/.zymatica/profiles/<profile>/...` under a
...
The ONLY import surface is `@zymatica/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.zymatica/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@zymatica/plugin-sdk`, `react`, and
