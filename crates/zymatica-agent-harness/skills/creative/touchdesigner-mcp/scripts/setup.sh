...
ZYMATICA_HOME_DIR="${ZYMATICA_HOME:-$HOME/.zymatica}"
ZYMATICA_CFG="${ZYMATICA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Zymatica config has twozero_td MCP entry ──
if [[ ! -f "$ZYMATICA_CFG" ]]; then
    echo -e " ${FAIL} Zymatica config not found at ${ZYMATICA_CFG}"
    manual_steps+=("Create ${ZYMATICA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ZYMATICA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Zymatica config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Zymatica config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$ZYMATICA_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ZYMATICA_CFG} manually"); }
    manual_steps+=("Restart Zymatica session to pick up config change")
fi
