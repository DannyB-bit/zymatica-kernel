...
metadata:
  zymatica:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, zymatica-video]
...
```
Zymatica Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${ZYMATICA_HOME:-$HOME/.zymatica}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Zymatica config (if missing)
...
3. **Restart Zymatica session** to pick up the new MCP server
