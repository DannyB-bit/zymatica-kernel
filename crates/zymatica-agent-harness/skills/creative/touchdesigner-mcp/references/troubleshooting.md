...
## 5. Zymatica Configuration

### Config location

`$ZYMATICA_HOME/config.yaml` (defaults to `~/.zymatica/config.yaml` when `ZYMATICA_HOME` is unset)
...
Restart the Zymatica session for changes to take effect. The MCP connection is
