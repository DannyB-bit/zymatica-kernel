...
| Change | Upstream | Zymatica |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `zymatica` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of Zymatica infra |
...
| File operations | Bash commands | Zymatica file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains Zymatica-specific adaptations.
