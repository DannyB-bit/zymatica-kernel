...
metadata:
  zymatica:
    ...
    related_skills: [claude-code, zymatica-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Zymatica terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Zymatica itself, `model.provider: openai-codex` uses Zymatica-managed Codex
OAuth from `~/.zymatica/auth.json` after `zymatica auth add openai-codex`. For the
...
## Zymatica Gateway Caveat

When invoking the Codex CLI from a Zymatica gateway/service context (for example,
