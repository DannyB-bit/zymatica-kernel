...
2. An event-loop heartbeat file at ``<ZYMATICA_HOME>/state/gateway.heartbeat`` so
...
def _process_zymatica_home() -> Path:
    """ZYMATICA_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<ZYMATICA_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_zymatica_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from zymatica_logging import drain_log_queue
