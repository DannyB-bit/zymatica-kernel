...
# dashboard's pre-write mutation validation (zymatica_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Zymatica is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_zymatica_home() / "sessions")
    ...
    # gateway behaves exactly as before — single ZYMATICA_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_zymatica_home() / "sessions"
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``zymatica config set gateway.multiplex_profiles true``.
            multiplex_profiles = nested_gateway.get("multiplex_profiles")
        # Operator override: GATEWAY_MULTIPLEX_PROFILES wins over config.yaml when
        # set to a recognized value. Hosted deployments (Zymatica Portal / Fly) stamp
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.zymatica/config.yaml (primary user-facing config)
    3. ~/.zymatica/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_zymatica_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through zymatica_cli.config.load_config, so without this a managed
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `zymatica config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``zymatica config set gateway.streaming.*``
                streaming_cfg = (
