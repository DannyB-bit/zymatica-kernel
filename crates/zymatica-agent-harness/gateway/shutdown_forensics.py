...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        zymatica_home_str = os.environ.get("ZYMATICA_HOME")
        if zymatica_home_str:
            takeover_path = Path(zymatica_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(zymatica_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded zymatica-agent but never re-ran ``zymatica setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../zymatica-gateway.service"
    ...
    # that's the common case for zymatica.
    ...
    # constant in zymatica_cli/gateway.py.
    headroom = 30.0
