...
from zymatica_constants import get_zymatica_dir, get_zymatica_home
...
PAIRING_DIR = get_zymatica_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from zymatica_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{ZYMATICA_HOME}/pairing`` while newer code/docs may
    write ``{ZYMATICA_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<ZYMATICA_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<ZYMATICA_HOME>/pairing/``
    directory (backward-compat for the ``zymatica pairing`` CLI).
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp ZYMATICA_HOME
        # and PairingStore may be constructed before the env is set.
        if profile:
            from zymatica_constants import get_zymatica_home
            self._dir = get_zymatica_home() / "profiles" / profile / "pairing"
        else:
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `zymatica` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> zymatica pairing approve ...` as root, "
                    "re-run with `docker exec -u zymatica <container> ...` and "
                    "chown the existing file to the zymatica user, or restart the "
