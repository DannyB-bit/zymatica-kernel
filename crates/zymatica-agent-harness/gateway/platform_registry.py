...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``zymatica setup`` display).
    ...
    # built-ins).  Used by ``zymatica gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `zymatica`
        # invocation -- including plain `zymatica chat`, which never touches any
        ...
        # `zymatica setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``zymatica setup``/``gateway status``, channel
