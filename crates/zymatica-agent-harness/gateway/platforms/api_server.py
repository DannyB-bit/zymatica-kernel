...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Zymatica-Session-Id header; opt-in long-term memory scoping via X-Zymatica-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Zymatica-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists zymatica-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Zymatica sessions
- POST /api/sessions               — create an empty Zymatica session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to zymatica-agent
...
def _zymatica_version() -> str:
    """Return the zymatica-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``zymatica_cli.__version__`` (covers editable /
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_zymatica_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted ZYMATICA_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # zymatica_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same Zymatica session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through zymatica-agent's AIAgent.
    """
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            return None
        try:
            from zymatica_cli.profiles import profiles_to_serve
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_zymatica_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
        from zymatica_cli.profiles import get_profile_dir
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Zymatica-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Zymatica-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Zymatica-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Zymatica-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Zymatica-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``zymatica sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_zymatica_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_zymatica_home())
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the zymatica-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Zymatica-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "zymatica-agent", "version": _zymatica_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "readiness": readiness,
            "platform": "zymatica-agent",
            "version": _zymatica_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list zymatica-agent and any configured model_routes aliases.
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Zymatica version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "zymatica.api_server.capabilities",
            "platform": "zymatica-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Zymatica AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Zymatica-Session-Id",
                "session_key_header": "X-Zymatica-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Zymatica sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Zymatica session row.
        ...
        return web.json_response({"object": "zymatica.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "zymatica.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "zymatica.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "zymatica.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Zymatica-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Zymatica-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "zymatica.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Zymatica-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Zymatica-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Zymatica-Session-Key.  This
        # is independent of X-Zymatica-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Zymatica-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Zymatica-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Zymatica-Session-Id rejected: "
            ...
            # conversation map to the same Zymatica session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``zymatica.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Zymatica-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Zymatica-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["zymatica"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Zymatica-Completed"] = "false"
            response_headers["X-Zymatica-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Zymatica-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Zymatica-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Zymatica-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Zymatica-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Zymatica-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: zymatica.tool.progress`` SSE event so
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    await response.write(
                        f"event: zymatica.tool.progress\ndata: {event_data}\n\n".encode()
                    )
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Zymatica-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Zymatica-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-Zymatica-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Zymatica-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "zymatica.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    def _api_key_passes_startup_guard(self) -> bool:
        ...
        try:
            from zymatica_cli.auth import has_usable_secret
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Zymatica-Relay
            ...
            # that is the exact surface the zymatica-0day campaign abused to write
            # ~/.zymatica/config.yaml and plant persistence. Sandboxing (Docker /
