...
There are two ways to add a platform to the Zymatica gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.zymatica/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Zymatica code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `zymatica gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `zymatica_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Zymatica core.
...
And add it to the `zymatica-gateway` composite:

```python
"zymatica-gateway": {
    "includes": [..., "zymatica-your-platform"]
}
```
...
## 12. Status Display (`zymatica_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zymatica_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zymatica_cli/ toolsets.py \
  --include="*.py" -l | sort -u
