...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Zymatica Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ZYMATICA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/zymatica), this function
    mirrors the bridge source to a writable ZYMATICA_HOME location and returns that
    ...
    # Try ZYMATICA_HOME location first
    zymatica_home = get_zymatica_home()
    zymatica_home_bridge = zymatica_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ZYMATICA_HOME if needed
    if zymatica_home_bridge.exists():
        return zymatica_home_bridge

    # Mirror the bridge source to ZYMATICA_HOME
    try:
        zymatica_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            zymatica_home_bridge,
            dirs_exist_ok=False,
        )
        return zymatica_home_bridge
    except Exception:
