...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.zymatica to the active profile home."""
    ...
    if raw == "~/.zymatica":
        return zymatica_home
    if raw.startswith("~/.zymatica/"):
        return zymatica_home / raw.removeprefix("~/.zymatica/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return zymatica_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under ZYMATICA_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from zymatica_constants import get_zymatica_home

    scripts_root = (get_zymatica_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.zymatica" or raw_text.startswith("~/.zymatica/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__zymatica_ignore__") is True
        ):
            return False, None
