...
Defenses 1 and 2 (the ``_ZYMATICA_GATEWAY`` guard on ``zymatica gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.zymatica.gateway")``,
...
State lives in ``<ZYMATICA_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_zymatica_home() / "gateway" / "restart_loop.json"
