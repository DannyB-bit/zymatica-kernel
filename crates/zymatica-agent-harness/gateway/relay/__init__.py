"""Relay/connector support package for the Zymatica gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``zymatica gateway enroll`` writes them to
    ``~/.zymatica/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``ZYMATICA_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``zymatica gateway enroll``).
...
def _resolve_relay_identity_token() -> str:
    ...
    ``zymatica gateway enroll`` CLI. Two modes, in precedence order:

      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         Zymatica Portal): when ``gateway.idp.token_url`` (or
      ...
      2. **Zymatica Portal** (default): ``resolve_nous_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — Zymatica Portal (default, unchanged behaviour).
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.zymatica/.env``.
    ...
    sets neither ``ZYMATICA_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``zymatica gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'zymatica'}"
