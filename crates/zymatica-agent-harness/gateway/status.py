...
The PID file lives at ``{ZYMATICA_HOME}/gateway.pid``.  ZYMATICA_HOME defaults to
``~/.zymatica`` but can be overridden via the environment variable.  This means
separate ZYMATICA_HOME directories naturally get separate PID files — a property
...
from zymatica_constants import get_zymatica_home, _get_platform_default_zymatica_home
...
_GATEWAY_KIND = "zymatica-gateway"
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_zymatica_home() / "gateway-starts.log"
...
def _get_process_zymatica_home() -> Path:
    """Return the process-level ZYMATICA_HOME, skipping context-local overrides.
    ...
    ``get_zymatica_home()`` honors ``_ZYMATICA_HOME_OVERRIDE`` contextvar used for
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ZYMATICA_HOME."""
    home = _get_process_zymatica_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ZYMATICA_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "zymatica" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Zymatica gateway lifecycle subcommand from a command line.
    ...
    test also matched ``zymatica_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\zymatica-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Zymatica's
    ...
    for token in tokens:
        ...
        if basename in ("zymatica-gateway", "zymatica-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "zymatica_cli.main" in joined
        or "zymatica_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("zymatica", "zymatica.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `zymatica gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Zymatica-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Zymatica gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a ZYMATICA_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.zymatica`` or ``$ZYMATICA_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``zymatica_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``ZYMATICA_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"zymatica_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit ZYMATICA_HOME= on the argv. ZYMATICA_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "zymatica_home=" in command_lc and f"zymatica_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `zymatica status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``ZYMATICA_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    ZYMATICA_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ZYMATICA_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``zymatica.service`` + ``zymatica-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different ZYMATICA_HOME. When two profile gateway
    # services share the same default ~/.zymatica (ZYMATICA_HOME not set
    ...
    # field is absent in markers written by older Zymatica versions; treat
    ...
    replacer_home = record.get("replacer_zymatica_home")
    if replacer_home is not None and replacer_home != str(_get_process_zymatica_home()):
        return False
    ...
    # ``zymatica gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_zymatica_home": str(_get_process_zymatica_home()),
