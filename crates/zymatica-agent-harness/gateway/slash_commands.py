...
Module-level run.py helpers a handler needs (``_zymatica_home``,
...
from zymatica_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: zymatica gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Zymatica commands.
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        """
        from zymatica_constants import display_zymatica_home
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`zymatica gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _zymatica_home
        ...
        try:
            ...
            atomic_json_write(
                _zymatica_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _zymatica_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Zymatica Agent version."""
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _zymatica_home, _load_gateway_config
        import yaml
        ...
        user_provs = None
        custom_provs = None
        config_path = (_command_profile_home or _zymatica_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from zymatica_cli.context_switch_guard import (
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Zymatica default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        config_path = _zymatica_home / 'config.yaml'
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_zymatica_home())
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _zymatica_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.zymatica/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``zymatica skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _zymatica_home
        from zymatica_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `zymatica skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.zymatica/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _zymatica_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _zymatica_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from zymatica_cli.session_listing import (
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # Run off the event loop. The helper gates on "a Zymatica account is logged in"
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `zymatica bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``zymatica debug share`` from the CLI.
        """
        import asyncio
        from zymatica_cli.debug import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Zymatica Agent to the latest version.

        Spawns ``zymatica update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``zymatica update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _zymatica_home, _resolve_zymatica_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Zymatica Agent')}"
        ...
        zymatica_cmd = _resolve_zymatica_bin()
        if not zymatica_cmd:
            return t("gateway.update.zymatica_cmd_not_found")

        pending_path = _zymatica_home / ".update_pending.json"
        output_path = _zymatica_home / ".update_output.txt"
        exit_code_path = _zymatica_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `zymatica update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `zymatica update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # zymatica_cmd is a list of argv parts we can pass directly
                ...
                zymatica_cmd_str = " ".join(shlex.quote(part) for part in zymatica_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {zymatica_cmd_str} update --gateway"
